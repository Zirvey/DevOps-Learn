...
export const omadaWirelessChapter: Chapter = {
  id: 'omada-wireless',
  slug: 'omada-wireless',
  title: 'Omada — Wi‑Fi и EAP',
  moduleId: 'omada',
  order: 2,
  duration: '12–15 часов',
  level: 'intermediate',
  description:
    'EAP650/EAP670, SSID, WPA3, RADIUS/NPS, guest portal, channel planning, spectrum analysis, IoT SSID, voice VLAN, roaming и troubleshooting',
  sections: [
    {
      title: 'Архитектура Wi‑Fi в Omada',
      content: `**Централизованное управление:** Controller push SSID config на все adopted AP. Клиент подключается к ближайшему AP; controller координирует roaming и RF settings.

**Компоненты:**
- **WLAN (SSID)** — логическая сеть, привязка к VLAN
- **EAP Access Point** — радио 2.4 / 5 / 6 GHz
- **RADIUS** — Enterprise auth (NPS)
- **Portal** — captive portal Guest
- **Gateway firewall** — изоляция Guest
...
\`\`\``,
    ...
    {
      title: 'EAP650 vs EAP670: выбор и размещение',
...
**Mounting:** ceiling center, 2.5–3 m height; ethernet home run; mesh = backup only.`,
    },
    {
      ...
      content: `**2.4 GHz — только каналы 1, 6, 11 (20 MHz):**
...
**DFS channels (52–144):** enable for more 5 GHz options; watch for radar detection channel switch events.`,
    ...
    {
      title: 'Site Survey: Ekahau и альтернативы',
...
**Acceptance report:** PDF heatmap + AP coordinate table + channel plan signed by customer.`,
    ...
    {
      title: 'Создание WLAN (SSID): пошаговый workflow',
...
| Embedded IoT | No | Maybe | Yes |`,
    ...
    {
      title: 'RADIUS и NPS: полная интеграция',
...
**Document:** inventory spreadsheet MAC → device name → VLAN → owner.`,
    ...
    {
      title: 'Dynamic VLAN через RADIUS attributes',
...
**Test:** sales user → 10.0.55.x; IT user → 10.0.56.x.`,
    ...
    {
      title: 'IoT SSID: проектирование и изоляция',
...
**Common failure:** voice on Guest SSID or without 802.11r → dropped calls at roam.`,
    ...
    {
      title: 'PEAP vs EAP-TLS: выбор для SMB',
...
**Omada:** proxies RADIUS, doesn't terminate 802.1X.`,
    ...
    {
      title: 'Guest Wi‑Fi и Captive Portal customization',
...
- **Welcome message:** «Добро пожаловать в Guest Wi‑Fi Office»
...
**Branding test:** mobile + desktop browser; check HTTPS cert warning on portal (use valid cert on controller if exposing portal externally).`,
    ...
    {
      title: 'Voucher Portal для посетителей',
      content: `**Workflow:**
...
**Rate limit per voucher:** 5 Mbps adequate for guests.`,
    ...
    {
      title: 'RF optimization: каналы, мощность, band steering',
...
**Transmit power:** reduce to minimize overlap — Auto often too high; try Medium manual tuning after survey.`,
    ...
    {
      title: 'Spectrum analysis и interference',
...
**IoT SSID:** disable 802.11r (breaks some devices).`,
    ...
    {
      title: 'Trunk VLAN к AP: критическая настройка',
...
**Diagnose:** AP Connected Clients VLAN tag; switch port VLAN membership.`,
    ...
    {
      title: 'Client Isolation и Rate Limiting',
...
**Verify:** two Guest clients cannot ping each other.`,
    ...
    {
      title: 'Mesh Wi‑Fi: когда использовать',
...
**Config:** Main AP wired + Mesh Enable; Satellite AP wireless.`,
    ...
    {
      title: 'Case study: кафе 10 users — Wi‑Fi',
...
**Pitfall:** Guest on same VLAN as POS — never; firewall + VLAN separation mandatory.`,
    ...
    {
      title: 'Case study: офис 50 — Wi‑Fi',
...
**Ops:** monthly RF scan; quarterly PSK rotation IoT if any.`,
    ...
    {
      title: 'Case study: склад 100+ — Wi‑Fi',
...
- [ ] Management VLAN not broadcast on guest AP`,
    ...
    {
      title: 'Lab walkthrough: SSID и portal за 15 шагов',
      content: `**Шаг 1.** Lab Controller + adopted AP (or document simulated).

**Шаг 2.** Wired Network VLAN 50 LAB-WIFI + DHCP.

**Шаг 3.** Wired Network VLAN 60 LAB-GUEST + DHCP.

**Шаг 4.** Switch AP port → AP-Trunk profile 50,60,99.

**Шаг 5.** RADIUS Profile (optional skip → WPA2-Personal for lab).

**Шаг 6.** Create WLAN «Lab-Corp» VLAN 50 WPA2-Personal PSK (lab simplification).

**Шаг 7.** Create WLAN «Lab-Guest» VLAN 60 Open.

**Шаг 8.** Portal → Local User → user guest/guest123.

**Шаг 9.** Attach portal to Lab-Guest.

**Шаг 10.** Firewall deny Guest VLAN → RFC1918.

**Шаг 11.** Connect phone to Lab-Corp → verify IP .50.x.

**Шаг 12.** Connect to Lab-Guest → portal login → verify IP .60.x.

**Шаг 13.** Ping internal gateway from Guest → must FAIL.

**Шаг 14.** Ping 8.8.8.8 from Guest → OK.

**Шаг 15.** Screenshot WLAN settings + export backup → lab report.

**Windows lab PC:** use ipconfig and ping for validation steps 11–14.`,
    },
    {
      title: 'FAQ: 10 частых вопросов (Wi‑Fi)',
      content: `**1. Сколько клиентов на EAP670?**
~40 practical; depends on bandwidth per client.

**2. Нужен ли отдельный SSID для 5 GHz?**
...
**Strong answers:** VLAN isolation, IGMP for multicast, trunk VLAN checklist, Ekahau vs free survey tools.`,
    ...
    {
      title: 'WLAN Groups: per-floor и per-building',
      content: `**WLAN Group** — набор SSID, broadcast только на AP в группе. Полезно когда разные здания или этажи нужны разные SSID.
...
**Pitfall:** AP not in any group with SSID applied to «All AP» vs specific group — verify Apply To settings.`,
    ...
    {
      title: 'mDNS/Bonjour и multicast-to-unicast',
      content: `**Problem:** Apple AirPrint, Chromecast, Bonjour не работает across VLANs или между Wi‑Fi clients with isolation.
...
**Security:** don't reflect mDNS Guest → Corp — prevents service enumeration attacks.`,
    ...
    {
      title: 'Outdoor и industrial AP considerations',
...
**Compliance:** outdoor coverage into public street may need acceptable use policy on Guest portal.`,
    ...
    {
      title: 'Wi‑Fi 7 (EAP773) и future planning',
...
**Don't deploy Wi‑Fi 7 only for marketing** — Wi‑Fi 6E/7 valuable when clients exist and spectrum clean.`,
    ...
    {
      title: 'RADIUS accounting и session tracking',
...
**Dashboard:** parse NPS logs into Graylog → Grafana «Wi‑Fi sessions per day» panel.`,
    ...
    {
      title: 'WPA3-Personal для Guest alternative',
...
Each scenario: document **detection → triage → fix → verify → post-mortem** in runbook wiki.`,
    ...
    {
      title: 'Ekahau-free workflow: пошаговый альтернативный survey',
      content: `**Полный workflow без Ekahau для офиса 50 человек (2 этажа):**

**Phase 1 — Predictive (2 hours):**
1. Получи floor plan PDF от арендодателя
2. Импорт в NetSpot (Mac/Windows) или распечатай A3
3. Отметь зоны: open space, meetings, kitchen (metal/appliances), server room
4. Размести виртуальные AP EAP670 каждые 10–12 m open space
5. Meetings: 1× EAP650 per 3 rooms cluster
6. Export screenshot predicted coverage

**Phase 2 — Pre-install validation (4 hours):**
1. Арендуй 2 EAP670 + PoE switch
2. Temp mount AP at planned positions (clamp mount, не финальный монтаж)
3. Walk test grid 5m с WiFi Analyzer Android:
...
**Document every ticket:** scope, BSSID, VLAN, RADIUS accept/reject, resolution — builds knowledge base.`,
    ...
    {
      title: 'Безопасность Wi‑Fi для SMB',
...
**Compliance audit:** quarterly firewall + Wi‑Fi isolation test documented.`,
  ...
  practice: [
    'Создай Corp SSID (WPA3-Enterprise) и Guest SSID (Portal) — VLAN 50 и 60',
    'Разверни NPS: RADIUS client Omada, policy Domain Users — test 802.1X',
    'Настрой WPA3-Transition SSID и протестируй legacy + modern clients',
    'Dynamic VLAN: две AD группы → два VLAN — verify IP assignment',
    'Guest portal: custom logo, 10 vouchers, rate limit 10 Mbps',
    'Firewall Guest test: ping internal FAIL, 8.8.8.8 OK',
    'Channel plan spreadsheet для 6 AP с calculator formula',
    'Ekahau alternative: NetSpot/Omada App walk survey одного этажа',

...
export const omadaOperationsChapter: Chapter = {
  id: 'omada-operations',
  slug: 'omada-operations',
  title: 'Omada — VPN, мониторинг и эксплуатация',
  moduleId: 'omada',
  order: 3,
  duration: '12–15 часов',
  level: 'intermediate',
  description:
    'OpenVPN, IPsec Azure, WireGuard, firewall export, dual WAN, SNMP MIB, Grafana, syslog, maintenance calendar, DR controller и day-2 ops',
  sections: [
    {
      title: 'Эксплуатационная модель Omada Gateway',
      content: `**Omada Gateway (ER7206/ER8411)** — edge router, firewall, VPN concentrator, DHCP/DNS для всех VLAN.
...
**As-built docs:** WAN IPs, VLAN table, firewall matrix, VPN PSK in vault + Confluence.`,
    ...
    {
      title: 'OpenVPN: remote access для сотрудников',
...
**Client:** OpenVPN Connect all platforms.`,
    ...
    {
      title: 'OpenVPN: пользователи и аутентификация',
...
**Troubleshooting:** Phase 1 = IKE/PSK/NAT; Phase 2 = subnet selectors mismatch.`,
    ...
    {
      title: 'IPsec VPN к Microsoft Azure',
...
**Common failure:** Azure requires specific crypto suite — mismatch = Phase 1 fail; use Azure «Reset» on connection after Omada change.`,
    ...
    {
      title: 'WireGuard: сравнение с Omada VPN',
...
**2025 recommendation:** OpenVPN or IPsec only; migrate L2TP users within 6 months.`,
    ...
    {
      title: 'Firewall: проектирование правил',
      content: `**Zone model:**
...
**Log denies** for audit — watch volume.`,
    ...
    {
      title: 'Firewall rule set: полный export example',
...
**Omada UI export:** full config in controller .bin backup; per-rule documentation manual or via Open API where supported.`,
    ...
    {
      title: 'NAT, Port Forwarding и Hairpin',
...
**Quarterly review:** remove stale forwards.`,
    ...
    {
      title: 'Dual WAN: failover и load balance',
...
**Document:** both public IPs, LTE SIM ICCID, ISP contacts.`,
    ...
    {
      title: 'DNS, DHCP и сетевые сервисы Gateway',
...
**Troubleshoot «internet OK, names fail»:** DNS reachability, Guest DNS leak, VPN suffix.`,
    ...
    {
      title: 'SNMP MIB мониторинг Gateway и Switch',
...
\`\`\``,
    ...
    {
      title: 'Grafana dashboard ideas для Omada',
...
**Controller Software:** also export JVM metrics via node_exporter on host for controller health panel.`,
    ...
    {
      title: 'Syslog и централизация логов',
...
**Wazuh:** optional SIEM on same syslog stream for correlation.`,
    ...
    {
      title: 'Maintenance calendar: годовой план',
...
**Calendar template (ICS):** recurring Sunday 02:00 firmware window quarterly; backup daily 03:00 automated.`,
    ...
    {
      title: 'Backup, restore и disaster recovery controller',
...
**RPO target SMB:** 24h daily backup. **RTO target:** 4h.`,
    ...
    {
      title: 'Change management для SMB',
      content: `**Process:**
...
**Rollback template:** «Restore backup DATE-TIME» or «Delete rule #N».`,
    ...
    {
      title: 'Day-2 operations: регулярные задачи',
...
**Escalation:** outage > 1h business impact → manager + ISP ticket parallel.`,
    ...
    {
      title: 'Интеграция с Active Directory и NPS',
...
- [ ] Grafana alerts firing correctly (test alert monthly)`,
    ...
    {
      title: 'Case study: кафе 10 users — operations',
...
**Support:** owner phone → IT contractor retainer for emergencies.`,
    ...
    {
      title: 'Case study: офис 50 — operations',
...
**On-call:** operator tier-1 → admin tier-2 → TP-Link support tier-3 with logs ready.`,
    ...
    {
      title: 'Case study: склад 100+ — operations',
...
**Feature check post-upgrade:** IPsec to Azure, OpenVPN export, hairpin NAT if used.`,
    ...
    {
      title: 'Lab walkthrough: VPN и firewall за 15 шагов',
      content: `**Шаг 1.** Lab Controller + adopted ER7206 (or simulated gateway docs).

**Шаг 2.** Create VLAN 20 + 60 Wired Networks with DHCP.

**Шаг 3.** Firewall rule 1: Deny Guest → 10.0.0.0/8.

**Шаг 4.** Firewall rule 2: Allow LAN → WAN.

**Шаг 5.** Test Guest ping internal — must FAIL (skip if no Guest client).

**Шаг 6.** Settings → VPN → OpenVPN → Create server pool 10.0.200.0/24.

**Шаг 7.** Split tunnel 10.0.0.0/8 enabled.

**Шаг 8.** Create local VPN user labuser.

**Шаг 9.** Export .ovpn profile.

**Шаг 10.** Firewall Allow VPN 200 → VLAN 20 RDP.

**Шаг 11.** Install OpenVPN Connect on lab laptop.

**Шаг 12.** Connect VPN → verify IP 10.0.200.x.

**Шаг 13.** Ping internal gateway 10.0.20.1 — OK.

**Шаг 14.** Document firewall rules as YAML export format.

**Шаг 15.** Backup controller .bin → snapshot lab state.

**Windows lab:** OpenVPN Connect for Windows on steps 11–13; PowerShell Test-NetConnection for ping tests.`,
    },
    {
      title: 'FAQ: 10 частых вопросов (operations)',
      content: `**1. OpenVPN vs IPsec для remote users?**
OpenVPN easier; IPsec better site-to-site and Azure.

**2. Сколько OpenVPN на ER7206?**
~16 concurrent — check datasheet for exact firmware.

**3. Поддерживает ли Omada WireGuard?**
No native — use external Linux VM if required.

**4. Как экспортировать firewall rules?**
Manual YAML doc + full .bin backup; Open API limited write/read ACL.

**5. Dual WAN load balance ломает 1C?**
...
**Strong answers:** failover for 1C, Azure crypto table, same IP restore, SNMP+syslog stack, Git-tracked firewall YAML.`,
    ...
    {
      title: 'Dynamic DNS (DDNS) для changing WAN IP',
...
**Azure IPsec note:** Azure Local Network Gateway typically needs static IP — use DDNS updater script on Azure side if office IP dynamic (less common enterprise).`,
    ...
    {
      title: 'Controller high availability: options и limits',
...
**Database:** controller embeds config in local DB — no external PostgreSQL cluster option.`,
    ...
    {
      title: 'PCI-DSS и compliance logging',
...
\`\`\``,
    ...
    {
      title: 'NetFlow/sFlow и traffic visibility limits',
...
**Keep runbooks in Confluence one page each — link from Grafana alert annotations.**`,
    ...
    {
      title: 'Эскалация и поддержка TP-Link',
...
**Severity guide:** P1 all users down; P2 site partial; P3 single user VPN; P4 cosmetic.`,
  ...
  practice: [
    'Настрой OpenVPN: pool 10.0.200.0/24, split tunnel, export .ovpn — connect with client',
    'RDP к workstation VLAN 20 через OpenVPN — verify firewall rule',
    'IPsec site-to-site Omada ↔ strongSwan Linux OR second Omada — ping tunnel',
    'IPsec к Azure VNet: document crypto match table Phase1/Phase2',
    'Создай firewall YAML export 13 rules — implement top 5 in lab gateway',
    'Port forward 443 → internal с source IP whitelist — test external',
    'Dual WAN failover test — measure time and packet loss',
    'SNMP v3 setup + snmpwalk 5 OIDs с gateway',
    'Syslog → Graylog/Loki — alert на VPN auth failure',
    'Grafana: dashboard 6 panels (WAN, CPU, VPN, devices, DHCP, denies)',
    'Backup controller + simulate restore на lab VM same IP',
    'Quarterly restore drill checklist — execute in lab',
    'WireGuard comparison doc: когда НЕ использовать на Omada',
    'Maintenance calendar ICS или spreadsheet на 12 месяцев',

...
export const opnsenseServicesChapter: Chapter = {
  id: 'opnsense-services',
  slug: 'opnsense-services',
  title: 'OPNsense — сервисы и плагины',
  moduleId: 'opnsense',
  order: 3,
  duration: '6–8 часов',
  level: 'intermediate',
  description:
    'Плагины os-*, Suricata IDS/IPS, Zenarmor, HAProxy + ACME, Unbound advanced, Captive Portal, Netflow, DDNS, RADIUS/LDAP, мониторинг и кейсы production',
  sections: [
    {
      title: 'Сервисы и плагины: модель расширения OPNsense',
      content: `**OPNsense** разделяет **core firmware** (firewall, routing, базовый Unbound) и **плагины** — дополнительные сервисы в формате FreeBSD packages с префиксом **os-***.
...
**Где управлять:**
- **System → Firmware → Plugins** — установка, удаление, версии
- **Services** menu — конфигурация после установки
- **System → Firmware → Updates** — обновление core отдельно от plugins
...
OPNsense Core (FreeBSD + pf + Unbound base)
    ...
    ├── os-nginx        → nginx reverse proxy (альтернатива HAProxy)
    ├── os-c-icap       → ICAP для ClamAV integration
    └── os-zabbix-agent → Zabbix agent package
\`\`\`

**Важно:** плагин = package + OPNsense plugin metadata (menu entries, config templates, migrations). Удаление плагина через GUI обычно чистит config sections; orphaned config — редко, но проверяй после uninstall.

**Production mindset:** не устанавливай «всё из каталога». Каждый плагин — процесс (CPU, RAM, logs, updates). Документируй installed plugins в runbook.`,
    },
    {
      title: 'Плагины os-*: установка, зависимости и lifecycle',
      content: `**Установка через GUI:**
1. System → Firmware → Plugins
2. Найди плагин (например **os-suricata**)
3. **+** Install → reboot не нужен для большинства, но Suricata/HAProxy — restart service

**CLI альтернатива (SSH):**
\`\`\`shell
pkg install os-suricata
pluginctl -g  # regenerate menu/config
\`\`\`

**Зависимости:** некоторые плагины требуют другие:
- **os-acme-client** часто используется с **os-haproxy** или **os-nginx**
- **os-c-icap** + ClamAV для antivirus scanning
- Zenarmor — отдельный vendor package, не всегда в default repo

**Версионирование:**
- Plugin version tied to OPNsense release (24.7, 25.1…)
- После major upgrade core — обнови все plugins в тот же maintenance window
- **Incompatible plugin** блокирует upgrade — read release notes

**Удаление:**
1. Disable service в GUI
2. Plugins → remove os-*
3. Verify: \`pkg info | grep os-\` на shell
4. Backup config.xml before/after

**Lock packages (опционально):** для стабильных production — фиксируй версии в change management; auto-update plugins без теста на lab — риск.

| Плагин | Типичный RAM overhead | CPU impact |
|--------|----------------------|------------|
| os-suricata | +500 MB – 2 GB | High на IPS |
| os-haproxy | +50–100 MB | Low–medium |
| os-zenarmor* | +200–500 MB | Medium |
| os-zabbix-agent | +20 MB | Minimal |

*Zenarmor licensing отдельно от OPNsense.`,
      code: {
        language: 'shell',
        caption: 'Проверка установленных os-* пакетов и статуса сервисов',
        code: `# список OPNsense plugins
pkg info | grep '^os-'

# статус plugin-managed services
...
configctl restart suricata`,
    ...
    {
      title: 'Suricata IDS/IPS: архитектура и размещение',
      content: `**Suricata** — signature-based **IDS** (detect) и **IPS** (inline block) на базе rule matching. Плагин **os-suricata** интегрирует daemon, rule updates, alerts UI.

**Поток трафика:**
\`\`\`
Packet on interface (WAN/LAN)
    → Suricata inspection engine
    → Rule match → Alert (IDS) или Drop/Reject (IPS)
    → Log → local / syslog / Eve JSON
\`\`\`

**Когда Suricata на OPNsense:**
- SMB без budget на FortiGuard IPS
- Нужны ET Open / custom rules
- Homelab / MSP CPE с единым стеком

**Когда НЕ Suricata alone:**
- >1 Gbps sustained с full IPS — нужен hardware NGFW или dedicated IDS appliance
- Требуется managed SOC без своего SIEM

**Placement strategy:**

| Interface | Сценарий | Риск |
|-----------|----------|------|
| **WAN** | Inbound attacks, exploit kits | False positives на legitimate traffic редко |
| **LAN** | East-west, lateral movement | Высокий FP rate, CPU load |
| **DMZ OPT** | Public services segment | Focused ruleset |
| **Multiple** | Defense in depth | CPU × N interfaces |

**Start recommendation:** IDS-only на WAN, 2 недели tuning → IPS enable selective rules.

**Отличие от Zenarmor:** Suricata = packet/signature IDS классика; Zenarmor = L7 application visibility + policy (другая модель, может complement).`,
    },
    {
      title: 'Suricata: режимы IDS, IPS и IPS с drop',
      content: `**Services → Intrusion Detection → Administration:**

| Mode | Описание | Production use |
|------|----------|----------------|
| **IDS** | Detection + alert, трафик проходит | Tuning phase, monitoring |
| **IPS** | Inline enforcement на matched rules | После whitelist tuning |
| **IPS with drop** | Active drop без state (aggressive) | Rare, specific threats |

**IDS mode:**
- Не влияет на connectivity
- Alerts в Suricata log + optional syslog
- Идеален для learning baseline

**IPS mode:**
- Suricata действует как inline filter (через netmap/ divert — implementation на FreeBSD)
- Matched rule с action drop/reject → пакет не проходит
- **Risk:** false positive = outage для legitimate app
...
- Office: ERP, custom ports, SaaS — FP nightmare без tuning

**Fail-open vs fail-close:** при crash Suricata трафик обычно проходит (fail-open) — проверь документацию версии; для critical compliance иногда external tap IDS preferred.`,
    },
    {
      title: 'Suricata: rulesets, ET Open и commercial feeds',
      content: `**Rulesets** — набор сигнатур (SID). Управление: Services → Intrusion Detection → Download.
...
- **POLICY** — P2P, games (если не security policy)
- **CHAT** — если не нужен
...
- Detect DNS tunneling patterns (limited без ML)

**Rule actions:** alert, drop, reject, pass — pass для whitelist IP/signature.

**Storage:** rules на disk — monitor space; full ET Open может быть 50k+ rules.`,
      code: {
        language: 'text',
        caption: 'Пример local custom Suricata rule (pass-list для update server)',
...
drop ip any any -> $HOME_NET any (msg:"DROP known scanner"; iprep:any,scanner,1,1; sid:9000002; rev:1;)`,
    ...
    {
      title: 'Suricata: выбор interfaces и hardware offload',
...
**VLAN-specific:** если Guest на отдельном VLAN interface — inspect Guest OPT для exfil detection.

**Hardware offload interaction:**
- NIC offload (LRO, TSO) иногда конфликтует с IDS — Suricata docs recommend disable offload на inspected interfaces
...
- [ ] Disk space for logs > 10 GB free`,
    ...
    {
      title: 'Suricata: alerts, Eve JSON и интеграция с SIEM',
      content: `**Alert sources:**
- GUI: Services → Intrusion Detection → Alerts
- Log files: \`/var/log/suricata/\`
- Eve JSON: structured events для ELK/Graylog/Wazuh
...
- Use dedicated log host; rate-limit на receiver
...
**Retention:** rotate logs weekly; Suricata verbose на busy WAN — GB/day.

**Don't alert fatigue:** email on every alert = ignored; use SIEM correlation rules.`,
      code: {
        language: 'shell',
        caption: 'Анализ Suricata Eve log на shell',
        code: `# top 10 signatures за последний час
...
grep '"action":"blocked"' /var/log/suricata/eve.json | tail -20 | jq .`,
    ...
    {
      title: 'Intrusion Detection: tuning и снижение false positives',
      content: `**False positive (FP)** — legitimate traffic matches attack signature. IPS FP = user-visible breakage.

**Tuning ladder:**

1. **Disable categories** — POLICY, P2P, CHAT если не нужны
2. **Threshold tuning** — некоторые rules имеют threshold в metadata
3. **Pass rules** — local.rules с pass action выше drop rules
...
**IDS on WAN before IPS:** mandatory для office networks.
...
**Metrics:** track alerts/day, blocked/day, CPU % — spike after rule update = review changelog ET.`,
    ...
    {
      title: 'Zenarmor: overview и сравнение с Suricata',
      content: `**Zenarmor** (ранее Sensei) — **L7 NGFW plugin** для OPNsense: application control, web filtering, device identification, cloud threat intel.

**Не замена Suricata 1:1** — разные engines:
- Suricata: signature IDS/IPS на packets
...
- Business: per device/month — factor в TCO vs FortiGate
...
**When choose Zenarmor:** need user-friendly app blocking without writing Suricata rules; MSP wants cloud-managed policies.`,
    ...
    {
      title: 'HAProxy: reverse proxy на OPNsense',
      content: `**os-haproxy** — reverse proxy / load balancer для publish internal services без direct port forward to fragile apps.
...
**Преимущества vs simple port forward:**
- SSL termination на OPNsense (central cert management)
...
**Limits:** OPNsense HAProxy not replacement for full F5/nginx-plus at massive scale; SMB 5–20 backends — OK.`,
      code: {
        language: 'text',
        caption: 'Фрагмент HAProxy backend config (conceptual — GUI generates)',
    ...
    {
      title: 'ACME Let\'s Encrypt: автоматизация сертификатов',
      content: `**os-acme-client** — ACME protocol (Let's Encrypt, ZeroSSL) для автоматического issuance и renewal TLS certificates.

**Challenge types на OPNsense:**

| Challenge | Когда | Requirement |
|-----------|-------|-------------|
...
**Failure alerts:** monitor renewal log; expired cert = outage Friday 00:00.`,
      code: {
        language: 'shell',
        caption: 'Проверка ACME certs и renewal log',
        code: `# список сертификатов ACME
ls -la /var/etc/acme-client/certificates/

# лог обновлений
tail -50 /var/log/acme/acme.log

# проверка expiry deployed cert
openssl s_client -connect vpn.office.com:443 -servername vpn.office.com 2>/dev/null | \\
  openssl x509 -noout -dates`,
      },
    },
    {
      title: 'Unbound advanced: blocklists, DNSSEC и sinkhole',
      content: `**Unbound** на OPNsense — recursive resolver для LAN. Advanced: blocklists, DNSSEC validation, local zones, forwarding.

**DNS blocklist (os-unbound-max или manual):**
...
- Ads blocking на work network — HR/legal may object
...
**Bypass risk:** clients using DoH/DoT direct to 8.8.8.8 — firewall block или DNS redirect (next sections).
...
**DNSSEC** — cryptographic chain of trust от root до domain. Unbound validates responses, blocks spoofed DNS.
...
**Monitoring:** \`unbound-control stats_noreset\` — validator stats; \`bogus\` counter should be low; spike = attack or misconfig.`,
      code: {
        language: 'shell',
        caption: 'Unbound DNSSEC и диагностика',
        code: `# статус Unbound
unbound-control status

# тест DNSSEC validation (должен быть ad = authenticated data)
...
drill dnssec-failed.org A @127.0.0.1`,
    ...
    {
      title: 'Unbound: host overrides, split DNS и forwarding',
...
**Migration tip:** document all overrides before moving DNS to dedicated appliance.`,
    ...
    {
      title: 'DNS over TLS (DoT) upstream и client policy',
...
**Use case:** ISP DNS snooping mitigation on WAN path; privacy для queries leaving OPNsense.
...
**DNS over HTTPS (DoH):** Unbound native DoH upstream evolving; check OPNsense version docs.`,
    ...
    {
      title: 'Dnsmasq vs Unbound: когда что использовать',
...
**Migration Dnsmasq → Unbound:** export overrides, replicate as Unbound host overrides, test resolution before cutover.`,
    ...
    {
      title: 'Captive Portal: гостевой Wi‑Fi и аутентификация',
...
**Alternatives:** Wi‑Fi controller guest (Omada, UniFi) may do portal on AP — avoid double portal.`,
    ...
    {
      title: 'ClamAV и ICAP: антивирусная фильтрация (ограничения)',
      content: `**ClamAV** через **os-c-icap** + ClamAV — HTTP/FTP antivirus scanning. Relevance **declining** в 2026: TLS everywhere limits clear-text scanning.
...
**Reality check:**
- **HTTPS** default — без SSL bump AV on firewall useless for web
...
**OPNsense strength:** IDS/Suricata malware rules complement (not replace) AV.`,
    ...
    {
      title: 'Netflow, Insight и traffic analytics',
...
**Alternatives:** mirror port to IDS appliance; switch sFlow from core switch — broader visibility than firewall-only flows.`,
    ...
    {
      title: 'Dynamic DNS (DDNS) и Wake on LAN (WoL)',
...
**Alternative:** IPMI/iDRAC for servers — more reliable than WoL.`,
      code: {
        language: 'shell',
        caption: 'Проверка DDNS статуса через log',
        code: `# лог dynamic DNS обновлений
...
ifconfig wan0 | grep inet`,
    ...
    {
      title: 'FreeRADIUS и LDAP: централизованная аутентификация',
...
**Testing:** Diagnostics → Authentication → test LDAP bind before enabling on VPN.`,
      code: {
        language: 'text',
        caption: 'Пример LDAP server config fields (GUI equivalent)',
...
Group attribute: memberOf`,
    ...
    {
      title: 'Zabbix и Telegraf: мониторинг OPNsense',
...
**Maintenance:** nginx plugin updates may require config migration — backup before upgrade.`,
    ...
    {
      title: 'Tor, VPN client packages: риски и политика',
      content: `**Outbound anonymity / bypass tools** в plugin repo — **os-tor**, OpenVPN client plugins, **os-wireguard** (legitimate VPN).
...
**Alternative:** route specific container through Tor on isolated VLAN lab — not on core firewall.`,
    ...
    {
      title: 'Безопасное обновление плагинов и core',
...
**Plugin-specific:** Suricata rule update separate from package update — can break IPS; schedule rules update off-peak after package stable.`,
    ...
    {
      title: 'Conflict avoidance: типичные конфликты плагинов',
...
- [ ] HA compatibility?`,
    ...
    {
      title: 'Практические лаборатории: сценарии отработки',
...
Document each lab: objective, steps, verification, rollback.`,
    ...
    {
      title: 'FAQ и Interview Q&A: сервисы и плагины OPNsense',
      content: `**Q: Suricata IPS сломал приложение — что делать первым?**
A: Переключи на IDS temporarily, find SID в alerts, add pass/suppress, re-enable IPS rule selectively.

**Q: HAProxy или port forward?**
A: Single backend simple app — port forward OK. Multiple backends, health checks, SNI — HAProxy.

**Q: Нужен ли Pi-hole если Unbound blocklist?**
...
**Q: Можно ли API управлять плагинами?**
...
A: Declining due to TLS; endpoint security primary; Suricata malware rules at network layer supplement.`,
    ...
    {
      title: 'Кейс: transparent proxy и SSL inspection (ограничения OPNsense)',
      content: `**Сценарий:** компания 80 users хочет «прозрачный прокси» с URL filtering без client PAC configuration.

**Expectation vs reality:**
- Modern HTTPS — без SSL bump proxy sees only SNI/IP (declining visibility)
...
**Lesson:** position OPNsense strength as DNS + IDS + routing; set SSL inspection expectations early with management.`,
    ...
    {
      title: 'Кейс: DNS filtering офиса 120 пользователей',
      content: `**Контекст:** офис 120 users, Guest Wi‑Fi, need malware DNS block, no Pi-hole budget, AD DNS for domain.
...
**Rollback plan:** DHCP DNS → DC only, disable blocklist — documented in runbook.`,
    ...
    {
      title: 'Кейс: Suricata IPS на WAN production MSP edge',
      content: `**Контекст:** MSP manages 40 OPNsense CPE VMs, asymmetric routing avoided, each site 20–50 users, WAN DSL/fiber.
...
**Client reporting:** monthly top signatures + blocked count PDF from Wazuh dashboard.`,
  ...
  practice: [
    'Установи os-suricata и os-haproxy; проверь pkg info | grep os- и configctl status для каждого сервиса',
    'Настрой Suricata IDS на WAN с ET Open; сгенерируй test scan alert и найди SID в Eve JSON',
    'После 20+ alerts составь suppress/pass list для 3 false positives и задокументируй SIDs',
    'Разверни HAProxy reverse proxy: WAN 443 → internal web 192.168.1.50:8080 с health check',
    'Настрой os-acme-client Let\'s Encrypt HTTP-01 для lab hostname; проверь авто-renew в acme.log',
    'Включи Unbound DNSSEC и blocklist malware; проверь drill +dnssec и blocked malware test domain',
    'Реализуй force DNS: firewall block outbound 53 кроме OPNsense; проверь bypass с 8.8.8.8 с клиента',
    'Настрой Captive Portal на Guest VLAN: click-through, изоляция RFC1918, тест с Wi‑Fi клиента',
    'Подключи Zabbix agent: template OPNsense, alert на остановленный Suricata, проверь на Zabbix server',
    'Сделай maintenance drill: backup config.xml, обнови plugin в lab, smoke test DNS/VPN/HAProxy, document rollback',
  ],
  resources: [

...
export const fortinetOperationsChapter: Chapter = {
  id: 'fortinet-operations',
  slug: 'fortinet-operations',
  title: 'FortiGate — эксплуатация и FortiSwitch/AP',
  moduleId: 'fortinet',
  order: 3,
  ...
  sections: [
    {
      title: 'Operational model для SMB',
      content: `**Эксплуатация FortiGate** в SMB офисе — это не только «настроил и забыл». Регулярные процессы:

| Процесс | Частота | Ответственный |
|---------|---------|---------------|
| Config backup verify | Ежедневно (automated) | Sysadmin |
| Log review (VPN, deny, UTM) | Еженедельно | Security / Sysadmin |
| FortiGuard license check | Ежемесячно | Sysadmin |
| Firmware security advisory | При release | Senior admin |
| Policy audit | Ежеквартально | Security |
| DR test (restore config) | Раз в полгода | Sysadmin |
| Capacity review (CPU, sessions) | Ежеквартально | Sysadmin |

**Документация (must-have):**
...
- Admin credentials в password manager (не в .conf файле!)

**Monitoring stack SMB:**
- FortiGate → SNMP → Zabbix/Prometheus (CPU, sessions, WAN)
- Logs → FortiAnalyzer или syslog (Graylog/ELK)
- Alerts → email/Telegram/Slack webhook
- Uptime → external ping (UptimeRobot) на public IP`,
    },
    {
      title: 'High Availability: Active-Passive',
      content: `**HA (High Availability)** — два FortiGate работают как пара: primary обрабатывает трафик, secondary в hot standby. При отказе primary → failover за 2–30 секунд.

**Active-Passive (SMB standard):**
- Один active, один passive
- Shared virtual MAC / virtual IP на LAN interfaces
- Session table синхронизируется (configurable)
- Одинаковые модели, firmware, licenses

**Требования:**
- 2× FortiGate (same model: 2× FG-100F)
- Dedicated HA link (direct cable между HA ports или dedicated VLAN)
- Secondary license (не обязательно full UTM, но recommended)
- Same firmware version

**Когда HA нужен SMB:**
- RTO < 5 min для internet/VPN
- Нет backup WAN (HA = hardware redundancy)
- Business critical: trading, medical, 24/7 operations

**Когда HA НЕ нужен (acceptable risk):**
- Офис 20–30 человек, downtime 1h = acceptable
- Есть backup 4G/LTE WAN + spare FG on shelf
- Budget constraint: HA pair FG-100F ≈ 2× cost

**Альтернатива HA:** single FG-100F + cold spare FG-60F (restore config from backup). RTO ~1-2 hours.`,
    },
    {
      title: 'HA configuration и failover test',
      content: `**Настройка HA (GUI):** System → HA → Mode: Active-Passive.
...
**Failover test (ежеквартально):**
1. Backup config
2. Notify users (brief VPN disconnect)
3. \`execute ha failover set 1\` на primary (force failover)
...
4. Never upgrade both simultaneously`,
      code: {
        language: 'text',
        caption: 'CLI: HA status и manual failover',
...
get system status | grep "Current HA mode"`,
    ...
    {
      title: 'FortiAnalyzer: архитектура и setup',
      content: `**FortiAnalyzer** — централизованный log management, reporting, compliance для Fortinet devices.

**Зачем SMB FortiAnalyzer:**
...
**Disk sizing:** 50 users office ≈ 5-15 GB/day logs (depends on log verbosity). Plan 1TB for 90 days.`,
    ...
    {
      title: 'Логирование: local, FortiAnalyzer, syslog',
...
- Disk usage trend`,
    ...
    {
      title: 'FortiSwitch и FortiLink',
      content: `**FortiSwitch** — managed switch, управляемый FortiGate через **FortiLink** protocol (proprietary, based on LLDP + CAPWAP-like).

**Преимущества FortiLink для SMB:**
- Единый GUI (FG) для firewall + switching
- VLAN assignment из FG
- 802.1X authentication через FG
...
**Quarantine VLAN:** compromised device → auto-move to quarantine port/VLAN via EMS tag.`,
    ...
    {
      title: 'FortiAP и wireless controller',
      content: `**FortiAP** — Wi-Fi access point, управляемый FortiGate (integrated wireless controller). Трафик: CAPWAP tunnel к FG или bridge mode.
...
| **Bridge** | Local switching на AP | Remote office, low bandwidth WAN |
...
**RF optimization:** automatic channel selection, TX power adjustment. Review annually.`,
    ...
    {
      title: 'SD-WAN: основы для SMB',
      content: `**SD-WAN на FortiGate** — intelligent routing across multiple WAN links.
...
**Не нужен SD-WAN если:** один WAN, или backup WAN только для emergency (static route failover достаточно).`,
      code: {
        language: 'text',
        caption: 'CLI: SD-WAN zone и performance SLA',
    ...
    {
      title: 'SD-WAN rules и SLA measurement',
      content: `**Performance SLA** измеряет качество каждого WAN:
- Latency (ms)
- Jitter (ms)
- Packet loss (%)
...
**Test restore ежеквартально** на VM или spare hardware.`,
      code: {
        language: 'text',
        caption: 'CLI: scheduled backup и restore',
...
execute restore config scp fg-20260710.conf backup@192.168.99.50:/fortigate/`,
    ...
    {
      title: 'Firmware upgrade: процедура production',
...
**Never:** auto-upgrade firmware in production. Signature updates — yes. Firmware — controlled process only.`,
    ...
    {
      title: 'Change control и ITSM integration',
      content: `**Change management для FortiGate:**
...
**Config annotation:** используй \`set comments\` в CLI или description в GUI для traceability.

**Monthly change review:** list all CHG tickets → verify config matches documentation.`,
    },
    {
      title: 'Diagnose commands: справочник',
      content: `**Ежедневный troubleshooting toolkit:**
...
diagnose sys session clear  # осторожно!
\`\`\`
...
**Важно:** \`diagnose debug enable\` — CPU intensive. Always \`diagnose debug disable\` after.`,
    },
    {
      title: 'Runbook: нет интернета в офисе',
      content: `**Runbook: P1 — полный outage интернета в офисе**

**Severity:** P1 | **RTO target:** 30 min

**Step 1 — Confirm scope (2 min):**
- Только office? Remote VPN users OK?
...
**Post-incident:** root cause analysis, update runbook.`,
    ...
    {
      title: 'Runbook: высокая CPU и session table',
      content: `**Runbook: FortiGate CPU > 85% или slow performance**
...
**Long-term:** upgrade hardware, optimize UTM policies, rate limiting.`,
    ...
    {
      title: 'Runbook: failover WAN и SD-WAN',
...
**Prevention:** monthly failover test (unplug wan1 cable, verify wan2 takes over in < 30s).`,
    ...
    {
      title: 'Capacity planning и health checks',
...
**FortiCare renewal:** 90 days before expiry — budget approval.`,
    ...
    {
      title: 'Incident response для FortiGate',
...
**Post-incident:** update policies, runbook, user training.`,
    ...
    {
      title: 'FortiManager: deployment для multi-site SMB',
      content: `**Когда FortiManager оправдан:**
- 3+ филиалов с identical base policies
...
execute central-mgmt register <FMG_IP> <FMG_SERIAL>`,
    ...
    {
      title: 'FortiAnalyzer: отчёты и compliance',
...
**Rollback action:** boot previous firmware partition or restore config on spare.`,
    ...
    {
      title: 'RMA process: замена hardware Fortinet',
...
**Cold spare program:** keep FG-60F on shelf with last config backup. RTO 1-2h without waiting RMA shipping.`,
    ...
    {
      title: 'Capacity planning: формулы и пороги',
...
**Quarterly review template:** fill table in ticket, compare to last quarter trend.`,
    ...
    {
      title: 'FortiOS 7.4 lab: operational baseline на VM',
...
**Deliverable:** operational runbook PDF for lab FG.`,
    ...
    {
      title: 'Справочник diagnose/debug (Operations)',
...
| \`diagnose debug crashlog read\` | Crash history |`,
    },
    {
      ...
      content: `**Клиент:** payment processor, PCI DSS audit.
...
**Remediation:** MFA enforced all VPN users within 48h. Geo-block SSL-VPN enabled.`,
    ...
    {
      title: 'FAQ: 12 частых вопросов (Operations)',
      content: `**1. FortiAnalyzer обязателен?**
Нет, но syslog + 90 day retention minimum for audit.

**2. HA нужен для 30-user office?**
Depends on RTO. Cold spare acceptable if RTO 2h OK.

**3. Как часто firmware upgrade?**
...
**Cross-chapter:** policies and VPN troubleshooting often combined in exam scenarios.`,
    ...
    {
      title: 'Interview Q&A: 10 вопросов (Operations)',
...
**Target SMB production:** Level 2 minimum, Level 3 for regulated industries.`,
    ...
    {
      title: 'Лабораторная: operational baseline',
      content: `**Цель:** настроить operational baseline на FortiGate VM.
...
8. **Runbooks:** write «нет интернета» 10-step checklist
...
**Deliverable:** operational runbook document for your lab FG.`,
  ...
  practice: [
    'Настрой syslog forwarding на Graylog/ELK в lab',
    'Создай scheduled encrypted backup на SCP/SFTP сервер',
    'Настрой email alerts: admin login fail, disk > 80%, HA failover',
    'Разверни FortiAnalyzer VM, подключи FG, создай VPN usage report',
    'Настрой SD-WAN: wan1 + wan2 с automatic failover, протестируй',
    'Подключи FortiSwitch к FG через FortiLink, назначь VLANs на порты',
    'Создай FortiAP SSID: Corp (VLAN20) + Guest (VLAN60)',
    'Выполни HA failover test (2 VM в lab) по formal procedure',
    'Runbook «нет интернета» — 10 шагов с CLI, протестируй в lab',
    'Runbook «CPU 100%» — diagnose commands, найди top talker',
    'Quarterly capacity review: заполни формулы для FG-60F',
    'Restore test: backup → factory reset VM → restore → verify',
    'FortiManager trial: добавь 2 FG VM, push policy package',
    'SOC lab: CEF syslog в Wazuh/Graylog, создай 3 alert rules',

...
export const officeDesignChapter: Chapter = {
  id: 'office-network-design',
  slug: 'office-network-design',
  title: 'Проектирование офисной сети',
  moduleId: 'office-network',
  order: 0,
  duration: '10–12 часов',
  level: 'beginner',
  description:
    'Топология SMB-офиса: интернет, firewall, core/access, серверная, IP-план, документация и типовые схемы',
  sections: [
    {
      title: 'Введение: проектирование офисной сети SMB',
      content: `**SMB office network** (10–100 пользователей) — баланс между enterprise-надёжностью и разумным бюджетом. Цель: безопасная, сегментированная, документированная сеть с запасом на рост.

**Принципы проектирования:**
1. **Defense in depth** — firewall на периметре, VLAN внутри
2. **Separation** — гости ≠ сотрудники ≠ серверы ≠ IoT
3. **Redundancy** — dual WAN, UPS, 2 DC, redundant uplinks где критично
4. **Scalability** — IP-план с запасом, PoE budget 30% headroom
5. **Documentation** — схема до go-live, не после аварии
...
| Handover | Docs, training, support contacts |`,
    ...
    {
      title: 'Топология: офис на 10 пользователей',
      content: `**Профиль:** стартап, open space, 1 этаж ~150 м², 1 сервер/NAS, remote backup.
...
[FortiGate 40F / Omada ER7206]  ← DHCP, DNS forward, VPN, Wi‑Fi controller
    │
[Managed PoE Switch 24p]  ← единственный switch
...
**Особенности 10 users:**
- Один switch достаточен
- Routing на firewall (no L3 switch needed)
- Windows Server или NAS для файлов + DC optional (Azure AD possible)
- DHCP на FortiGate или Windows`,
    },
    {
      title: 'Топология: офис на 50 пользователей',
      content: `**Профиль:** 2 этажа, 800 м², AD + 2 DC, IP-телефония, 6 AP, серверная 12U.
...
Inter-floor: Fiber OM4 2-core между этажами (или copper 10G if short)
\`\`\`

**Оборудование:**
| Устройство | Спецификация |
|------------|--------------|
| Firewall | FG-80F–100F, 1G WAN |
| Core | 24p L2+ или L3, 2×SFP+ |
| Access | 2× 48p PoE, 370W each |
| AP | 6× Wi‑Fi 6, ceiling mount |
| Fiber | OM4 patch между floors |
| UPS | 2000 VA online для rack |

**Серверная:** 12–18U rack, PDU metered, patch panel Cat6, fiber panel.`,
    },
    {
      title: 'Топология: офис на 100 пользователей',
      content: `**Профиль:** 3 этажа или 2000 м², 100 workstations, 20 VoIP, 12 AP, full AD, возможно branch VPN.
...
**Масштабирование:**
- /23 или /22 для workstations
- Dedicated management VLAN 99
- NetBox/phpIPAM обязателен
- 24×7 monitoring PRTG/Zabbix
- On-call rotation network team`,
    },
    {
      title: 'IP-план: универсальная схема VLAN',
      content: `**Базовая матрица VLAN (масштабируется 10–100 users):**

| VLAN ID | Name | Subnet (пример) | Hosts | Назначение |
|---------|------|-----------------|-------|------------|
...
**Для 10 users:** можно упростить до 5 VLAN (10,20,50,60,99) с /24 each.`,
    },
    {
      title: 'IP-план по размеру офиса',
      content: `**10 пользователей — компактный план:**
...
**50 пользователей — стандартный план** (см. матрицу выше, /24–/23)

**100 пользователей — расширенный:**
| VLAN | Subnet | Примечание |
|------|--------|------------|
| 20 Workstations | 192.168.20.0/22 | 1022 hosts |
| 50 WiFi Corp | 192.168.50.0/23 | 510 hosts |
| 30 VoIP | 192.168.30.0/23 | рост call center |

**Документ IPAM:** Excel, NetBox или phpIPAM — VLAN, subnet, gateway, DHCP scope, DNS, description, owner.`,
    },
    {
      title: 'Выбор и размерирование оборудования',
      content: `| Размер | Firewall | Core Switch | Access | AP | PoE budget |
|--------|----------|-------------|--------|-----|------------|
...
**AP sizing:** 1 AP per 80–120 м² open space, 1 per 4–6 rooms dense walls.`,
    },
    {
      title: 'Физическая прокладка и кабельная система',
      content: `**Структурированная кабельная система (СКС):**
...
| Тип | Применение |
|-----|------------|
| Cat6 UTP | Workstations, 1 Gbps, до 55m 10G |
| Cat6a | 10 Gbps to desk (trading floors) |
| OM3/OM4 fiber | Inter-floor, rack uplinks, long runs |
| OS2 single-mode | Campus, >300m |

**Серверная / MDF:**
- 42U rack (100 users) или 12U (10 users)
- Vertical cable managers обязательны
- PDU metered (C13/C19)
- UPS: 30 min minimum runtime at load
- Hot/cold aisle или fan if no CRAC
- Grounding per local code

**Маркировка:**
- Порт switch F2-SW01-P24 = Floor2 Patch panel port 24 = Desk F2-042
- Цвета патч-кордов: blue=user, red=uplink, yellow=server`,
    },
    {
      title: 'Оптоволокно: когда и как',
      content: `**Fiber vs copper uplink:**

| Критерий | Fiber OM4 | Copper Cat6a |
|----------|-----------|--------------|
| Distance | До 400m @ 10G | До 55m @ 10G |
| EMI immunity | Отлично | Зависит от среды |
| Cost | Выше (SFP modules) | Ниже |
| Inter-building | Да | Нет outdoor |

**Типовые сценарии fiber:**
- Между этажами (MDF ↔ IDF)
...
- Media converter — только если unavoidable

**Тестирование:** light level meter, или link lights minimum. Document dBm in handover.`,
    },
    {
      title: 'Dual WAN и отказоустойчивость интернета',
...
**Практика SMB 50 users:**
- Primary: business fiber 100/100 Mbps
- Backup: LTE 50 Mbps или второй ISP
- Failover time: 30–60 sec acceptable
- Document ISP NOC phones in runbook

**Публичные IP:**
- Static для VPN endpoint
- Mail relay — осторожно (SPF/DKIM)
- Port forwarding minimal (VPN only preferred)`,
    },
    {
      title: 'Серверная и инфраструктурные сервисы',
      content: `**Минимальный серверный стек 50 users:**

| Сервис | Размещение | VLAN |
|--------|------------|------|
...
**10 users alternative:** Azure AD + Intune, файлы SharePoint/OneDrive — on-prem DC optional.

**Виртуализация:** Hyper-V on 1 host (50 users) or 2-node cluster (100 users).`,
    },
    {
      title: 'Безопасность на уровне дизайна',
...
- No direct internet to switch GUI`,
    ...
    {
      title: 'Документация сети: обязательный пакет',
...
**NetBox** — open-source IPAM + DCIM, рекомендуется с 50 users.`,
    },
    {
      title: 'Процесс приёмки нового офиса',
...
- [ ] Admin training 2h session`,
    ...
    {
      title: 'Бюджет и типовые ошибки проектирования',
      content: `**Типовые ошибки:**

| Ошибка | Последствие | Prevention |
|--------|-------------|------------|
| Один VLAN | Security nightmare | Segment day 1 |
...
**Budget ratio ориентир (50 users):**
- Cabling 30%
- Switches + AP 35%
- Firewall 15%
- Servers 15%
- UPS/rack 5%`,
    ...
    {
      title: 'Сравнительная таблица: 10 vs 50 vs 100',
      content: `| Параметр | 10 users | 50 users | 100 users |
|----------|----------|----------|-----------|
| Switches | 1 | 3–4 | 5–8 |
| AP | 2 | 6 | 12 |
...
| Staff | MSP / 1 IT | 1 network part-time | Dedicated network |`,
    ...
    {
      title: 'BoM: 3 размера офиса с ценами',
...
**100 users (~€35-55k):** FG-100F HA €4000, L3 stack €6000, AP×12 €2400, dual WAN €500/mo, 2 IDF €8000, NetBox free, certs €1500.`,
    },
    {
      ...
      content: `Схема серверной (42U rack, вид спереди):
...
| U1  PDU         |
+---------------+
Cold aisle — спереди | Hot aisle — сзади`,
    },
    {
    ...
    {
      title: 'Лабораторная Design (16 шагов)',
...
Expanded labs FAQ troubleshooting case studies CompTIA mapping interview templates; aligned duration 10-12 hours; 15 practice 6 resources; RU language; slugs preserved.`,
    ...
    {
      title: 'Резюме главы',
      content: `1. **Топология** масштабируется: 10 (single switch) → 50 (core+access) → 100 (stack+IDF)
2. **VLAN matrix** — servers, users, VoIP, print, Wi‑Fi, guest, IoT, mgmt
3. **IP-план** — резерв .1–.50 infra, DHCP .100–.250
4. **Cabling** — Cat6 horizontal, fiber inter-floor, labeling mandatory
5. **Dual WAN** — SD-WAN failover с 50 users
6. **Documentation** — до go-live, NetBox, config backup
7. **Acceptance** — test WAN failover, Wi‑Fi survey, cable certification

> Следующая глава: **VLAN и коммутаторы** — trunk, STP, LAG, troubleshooting.`,
    },
  ],
  practice: [

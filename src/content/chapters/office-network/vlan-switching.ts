...
export const vlanSwitchingChapter: Chapter = {
  id: 'vlan-switching',
  slug: 'vlan-switching',
  title: 'VLAN и коммутаторы',
  moduleId: 'office-network',
  order: 1,
  duration: '10–12 часов',
  level: 'intermediate',
  description:
    '802.1Q, trunk и access порты, inter-VLAN routing, STP, LAG и настройка managed switches в офисе',
  sections: [
    {
      title: 'Зачем VLAN в офисе',
      content: `**VLAN (Virtual LAN, IEEE 802.1Q)** — логическая сегментация одного физического switch fabric на изолированные broadcast domains.

**Без VLAN:**
- Все устройства в одном L2 domain
- Broadcast storm затрагивает всех
- Гость подключается в розетку → видит серверы
- Невозможно QoS для VoIP
- MITM риски в open office

**С VLAN:**
- Broadcast изолирован внутри VLAN
- Firewall policies между сегментами
- VoIP priority (VLAN 30 + QoS)
- Guest internet без доступа к LAN
- Логическое перемещение user без перепрокладки кабеля

**802.1Q tag:** 4 bytes в Ethernet frame — VLAN ID (1–4094), priority (PCP).`,
    },
    {
      title: 'VLAN matrix для офиса 50 пользователей',
      content: `**Полная матрица VLAN и политик:**
...
**Native VLAN:** не использовать VLAN 1 — назначь 999 unused как native на trunks.`,
    },
    {
      title: 'Access vs Trunk порты',
      content: `| Тип | Tagged VLANs | Untagged | Подключение |
|-----|--------------|----------|-------------|
| **Access** | 0 | 1 (PVID) | PC, printer, IP phone (data) |
| **Trunk** | Multiple | Native (optional) | Switch↔switch, switch↔FW, switch↔AP |

**Пример port plan (48p access switch):**
...
- или dedicated port per device (проще troubleshoot)

**Native VLAN mismatch** между switches — leak traffic, security risk. Согласуй на всех trunks.`,
    },
    {
      title: 'Inter-VLAN routing на FortiGate',
      content: `**FortiGate как L3 router между VLAN:**

Каждый VLAN = interface на FG:
...
**DHCP per VLAN:** FortiGate DHCP server OR relay to Windows DC.`,
    ...
    {
      title: 'Inter-VLAN routing на L3 switch',
      content: `**Когда L3 switch (core):**
...
- ACL on switch or FG for east-west filtering`,
    },
    {
      ...
      content: `**STP (802.1D) / RSTP (802.1w)** — предотвращает L2 loops при redundant links.
...
**Петля без STP:** broadcast/multicast storm, MAC table flapping, network «умирает» за 10–30 секунд.

**Петля с STP:** one port blocked — redundancy without loop.`,
    },
    {
      title: 'Link Aggregation (LAG / LACP)',
      content: `**802.3ad LACP** — объединение 2–8 физических линков в один logical channel.
...
**Static LAG (no LACP)** — только если vendor requires, less flexible.`,
    },
    {
      title: 'QoS для VoIP и критичных сервисов',
      content: `**QoS layers:**
...
**Wi‑Fi:** WMM voice priority for Teams/Skype calls.`,
    ...
    {
      title: 'Настройка managed switch: пошаговый runbook',
...
18. Save config + backup to TFTP/Git`,
      code: {
        language: 'text',
        caption: 'Пример CLI (Cisco-like, концепт для любого L2)',
        code: `vlan 10
 name SERVERS
vlan 20
 name WORKSTATIONS
...
interface GigabitEthernet1/0/48
 ...
 switchport trunk native vlan 999`,
    ...
    {
      title: 'Omada и FortiSwitch: практические заметки',
...
**Выбор:** standalone Omada cheaper SMB; FortiSwitch if already FortiGate ecosystem.`,
    },
    {
      title: 'Port security и защита access layer',
...
**802.1X on wired:** NPS auth before port opens (advanced, see Wi‑Fi chapter for RADIUS).`,
    },
    {
      ...
      content: `| Симптом | Проверка 1 | Проверка 2 | Проверка 3 | Fix |
|---------|------------|------------|------------|-----|
...
- show vlan brief`,
    ...
    {
      title: 'Диагностика: пользователь не видит сервер',
...
**Traceroute** from user to server — где обрыв?
...
**Document in ticket:** each step result before escalation.`,
    ...
    {
      title: 'Stacking и redundancy access layer',
...
- Config change (if supported)`,
    ...
    {
      title: 'Миграция: от flat network к VLAN',
      content: `**План миграции без downtime (weekend):**
...
**Update on every change** — stale docs worse than none.`,
    ...
    {
      title: 'VLAN hopping: риски и защита',
      content: `**VLAN hopping** — атака перехода из одного VLAN в другой через подмену 802.1Q tags или exploitation double-tagging.

**Векторы:**
- **Switch spoofing** — attacker подключает свой switch, притворяется trunk
- **Double tagging** — frame с двумя VLAN tags, outer tag снимается на первом switch

**Защита (обязательно в офисе):**
| Мера | Настройка |
|------|-----------|
...
| Management VLAN | Отдельный, ACL на FG |
| No user trunk | Политика: users never get trunk port |

**Double tagging mitigation:** native VLAN не должен совпадать с user VLAN; не использовать VLAN 1.`,
    },
    {
      title: 'Практический кейс: офис 50 users end-to-end',
      content: `**Сценарий:** новый офис, 2 этажа, 50 workstations, 8 VoIP, 6 AP, 4 printers.

**Шаг 1 — Core switch SW-CORE-01:**
- Ports 47-48 LACP trunk to FortiGate
- STP priority 4096 (root)
- Management VLAN 99: 192.168.99.2

**Шаг 2 — Access SW-F1-01 (floor 1):**
...
**Документ:** port matrix Excel + Visio diagram в «книге сети».`,
    },
    {
...
Expanded labs FAQ troubleshooting case studies CompTIA mapping interview templates; aligned duration 10-12 hours; 15 practice 6 resources; RU language; slugs preserved.`,
    ...
    {
      title: 'Резюме главы',
      content: `1. **VLAN matrix** — сегментация servers, users, VoIP, print, Wi‑Fi, guest, IoT, mgmt
2. **Access vs trunk** — правильный порт = 50% troubleshooting
3. **Inter-VLAN routing** — FortiGate или L3 core + policies
4. **RSTP + BPDU guard** — защита от петель
5. **LACP** — uplink redundancy и bandwidth
6. **QoS** — VoIP VLAN приоритет
7. **Troubleshooting matrix** — systematic checks

> Следующая глава: **DHCP и DNS** — scopes, relay, split-horizon.`,
    },
  ],
  practice: [

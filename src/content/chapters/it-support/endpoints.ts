...
export const endpointSupportChapter: Chapter = {
  id: 'endpoint-support',
  slug: 'endpoint-support',
  title: 'Поддержка рабочих станций',
  moduleId: 'it-support',
  order: 2,
  duration: '10–12 часов',
  level: 'beginner',
  description:
    'Windows 10/11 troubleshooting, Outlook, Teams, принтеры, MDM Intune, onboarding ПК и типовые desktop задачи',
  sections: [
    {
      title: 'Стандартный образ рабочей станции (Golden Image)',
      content: `**Golden image** — эталонная конфигурация ПК, одинаковая для всех сотрудников. Снижает время onboarding и упрощает troubleshooting.

**Стандартный стек Windows 11 Enterprise:**

| Компонент | Настройка |
|-----------|-----------|
| ОС | Windows 11 Pro/Enterprise 23H2+ |
| Join | Azure AD Join + Intune (или Hybrid AD) |
| Шифрование | BitLocker (TPM 2.0) |
...
| **Manual** | Только lab / исключения |

**Не ставь вручную 50 ПК** — автоматизация окупается после 10–15 машин.

**Golden image checklist (20 пунктов):**
...
20. Test user login validation`,
    ...
    {
      title: 'Windows 11: архитектура и отличия от Windows 10',
      content: `**Ключевые отличия для support:**

| Аспект | Windows 10 | Windows 11 |
|--------|------------|------------|
| TPM | Рекомендуется | **Обязателен** 2.0 |
| Secure Boot | Рекомендуется | **Обязателен** |
| CPU | Широкий список | Ограниченный (Intel 8th+, AMD Zen+) |
| UI | Start left | Start center, rounded corners |
| Teams | Classic + optional | Chat from taskbar (можно убрать) |
...
**Частые блокеры миграции Win10→11:**
...
**Версии Windows 11 (понимать в тикете):**
- 22H2, 23H2, 24H2 — feature updates
- Build number в winver — для KB compatibility`,
    },
    {
      title: 'Диагностика Windows 11: базовый toolkit',
      content: `**Первые 5 минут любого desktop тикета:**

1. **Перезагрузка** — не шутка, pending reboot ломает 30% софта
...
**Task Manager — на что смотреть:**
| Вкладка | Проблема | Сигнал |
|---------|----------|--------|
| Performance | Медленный ПК | CPU 100%, Disk 100%, RAM > 90% |
| Startup | Долгая загрузка | 20+ enabled apps |
| Processes | Зависание | One app 4+ GB RAM |
| Users | «Кто залогинен» | Disconnected RDP sessions |

**Event Viewer — фильтры:**
...
**Reliability Monitor:** Control Panel → Security and Maintenance → Maintenance → View reliability history — timeline crashes.`,
      code: {
        language: 'powershell',
        caption: 'Быстрая диагностика Windows 11',
...
Test-NetConnection dc01.company.local -Port 389`,
    ...
    {
      title: 'Windows 11: углублённый troubleshooting',
      content: `**Медленный ПК — decision tree:**
...
**System file repair (порядок важен):**
1. DISM — repair Windows image
2. SFC — repair system files
3. CHKDSK — disk errors (schedule reboot)

**Сброс сети:** Settings → Network → Advanced → Network reset (last resort, removes VPN profiles).
...
**Blue Screen (BSOD):**
- Запиши stop code (CRITICAL_PROCESS_DIED, etc.)
...
chkdsk C: /f`,
    ...
    {
      title: 'Учётные записи: Azure AD, Hybrid и локальные',
      content: `**Типы join:**

| Тип | Где учётка | Типичный сценарий |
|-----|------------|-------------------|
| **Azure AD Join** | Cloud only | Remote, modern workplace |
| **Hybrid Azure AD Join** | On-prem AD + Azure AD sync | Legacy GPO + cloud |
| **Domain Join** | On-prem AD only | Старые среды |
...
**Профиль пользователя corrupt:**
- Создай temp admin, login as user → if fails, new profile
- Copy data from C:\\Users\\old.name to new
- Delete old profile in System Properties`,
    },
    {
      title: 'Microsoft Outlook: полный troubleshooting guide',
      content: `**Архитектура:** Outlook desktop → MAPI/Exchange Online or on-prem → mailbox in cloud/Exchange.

**Типовые проблемы и решения:**

| Симптом | Диагностика | Решение |
|---------|-------------|---------|
| Не отправляет | Outbox stuck, offline | Work offline off, delete stuck, check quota |
| Не получает | Rules, Focused Inbox | Disable rules test, check Junk |
...
**Offline mode:** Send/Receive tab → Work Offline (если горит) — отключи.
...
**scanpst.exe:** C:\\Program Files\\Microsoft Office\\root\\Office16\\SCANPST.EXE — для PST/OST errors.
...
Get-ItemProperty 'HKCU:\\Software\\Microsoft\\Office\\16.0\\Outlook\\Profiles\\*' -ErrorAction SilentlyContinue`,
    ...
    {
      title: 'Microsoft Teams: аудио, видео и производительность',
      content: `**New Teams vs Classic:** New Teams — отдельное приложение, другой cache path.

**Типовые проблемы:**

| Симптом | Решение |
|---------|---------|
| Нет звука | Settings → Devices → correct speaker/mic, test call |
| Микрофон не работает | Privacy → Microphone access for Teams |
| Камера black | Privacy → Camera, close other apps using cam |
| Высокий CPU | Hardware acceleration off, close GPU-heavy apps |
| Не входит | Clear cache, re-login, check MFA |
| Звонки обрываются | Network, VPN split tunnel, UDP blocked |
...
**VPN + Teams:** Full tunnel часто ухудшает качество — **split tunnel** для Teams media рекомендуется.

**Teams Rooms / shared:** отдельный playbook, firmware updates.`,
    },
    {
      title: 'OneDrive: синхронизация и Known Folder Move',
      content: `**OneDrive for Business** — sync client для SharePoint/OneDrive storage.
...
**Known Folder Move (KFM):** Desktop, Documents, Pictures → OneDrive. **Backup** перед troubleshoot!
...
**Share sync issues:** check permissions in SharePoint admin, sync library again.`,
    ...
    {
      title: 'Принтеры: сетевая печать и troubleshooting',
      content: `**Архитектура печати SMB:**
- Print server (Windows Server) или direct IP printers
- GPO или Intune deploy printer connections
...
**IP reservation:** DHCP reservation for printer MAC → stable IP for GPO and firewall.`,
    ...
    {
      title: 'Microsoft Intune и MDM: управление устройствами',
      content: `**Intune (Endpoint Manager)** — cloud MDM/MAM для Windows, iOS, Android.
...
Get-WinEvent -LogName 'Microsoft-Windows-DeviceManagement-Enterprise-Diagnostics-Provider/Admin' -MaxEvents 20`,
    ...
    {
      title: 'Onboarding нового ПК: пошаговый runbook',
      content: `**Pre-ship (до получения user):**
...
- [ ] Welcome KB sent`,
    ...
    {
      title: 'Offboarding и утилизация устройств',
      content: `**Leaver — день увольнения (строго по HR trigger):**
...
**Re-provision:** Wiped device → Autopilot reset → ready for next user.`,
    ...
    {
      title: 'Периферия: док-станции, мониторы, гарнитуры',
...
- Driver update for GPU`,
    ...
    {
      title: 'Безопасность рабочей станции',
...
- Revoke refresh tokens Azure AD`,
    ...
    {
      title: 'macOS в корпоративной среде (кратко)',
      content: `**Jamf / Intune for macOS** — если в компании есть Macs.
...
**Эскалация:** если нет Mac specialist — не экспериментируй, escalate L2.`,
    },
    {
      title: 'Матрица troubleshooting: desktop',
      content: `| Симптом | L1 checks (15 min) | Escalate L2 if |
|---------|-------------------|----------------|
...
**Всегда документируй** в тикете: версия ОС, build, app version, exact error text.`,
    },
    {
      title: 'Windows 11 24H2: новые features для support',
    ...
    {
      title: 'Autopilot step-by-step (15 шагов)',
    ...
    {
      title: 'Лабораторная Endpoint (16 шагов)',
...
**Glossary supplement:** MTTD mean time to detect; MTBF mean time between failures; CAB change advisory board; CSI continual service improvement; LOB line of business; SSPR self-service password reset; TAP temporary access pass Azure; ODJ offline domain join; ESP enrollment status page; PKI public key infrastructure; SCEP cert enrollment protocol; MDM mobile device management; UEM unified endpoint management; NAC network access control; captive portal guest auth page; RTO recovery time objective; RPO recovery point objective; BIA business impact analysis; DRP disaster recovery plan; BCP business continuity plan; SPOF single point of failure; HA high availability; VRRP virtual router redundancy; HSRP Cisco FHRP; ICMP ping traceroute; ARP address resolution; NIC network interface; DHCPv6 stateful stateless; AAAA IPv6 DNS; dual stack IPv4 IPv6 simultaneous; split tunnel VPN corp only routes; full tunnel all traffic VPN; SSL VPN web portal; IPsec site to site; MFA push TOTP FIDO2; Conditional Access Azure policies; PIM privileged identity management; DLP data loss prevention; EDR endpoint detection response; SOC security operations center; SIEM log aggregation; IOC indicator of compromise; CVE common vulnerabilities; CVSS severity score; Ransomware encrypt extort; BEC business email compromise; Phishing simulation training; Zero Trust never trust always verify; Least privilege minimum access required; Defense in depth layered security; DMZ demilitarized zone perimeter; NAT PAT overload translation; ACL access control list; IPS intrusion prevention inline; IDS intrusion detection passive; WAF web application firewall; DDoS distributed denial of service; QoS quality of service voice video data; DSCP diffserve code point marking; CoS class of service layer2; LLDP link layer discovery; CDP Cisco discovery protocol proprietary; STP spanning tree loop prevention; RSTP rapid STP; MSTP multiple STP; BPDU bridge protocol data unit; LACP link aggregation control protocol; PoE+ 802.3at 30W; PoE++ 802.3bt 60-90W; SFP GBIC fiber transceiver; SMF single mode fiber long range; MMF multimode fiber short range; OM4 fiber standard 10G; Cat6 Cat6a copper standards; TIA-568 wiring standard; Fluke cable certifier test; IDF intermediate distribution frame; MDF main distribution frame; UPS PDU power infrastructure; FG FortiGate; NPS RADIUS Windows; AD DS domain services; GPO group policy; OU organizational unit; AAD Azure Active Directory Entra; Intune MEM endpoint manager; Autopilot zero touch deploy; BitLocker TPM encryption; WUfB Windows update for business; ESP enrollment status page; OST Outlook offline data file; OWA Outlook web app; Teams NG client; OneDrive sync client; SharePoint Online collaboration; M365 Microsoft 365 suite; EXO Exchange Online; SPO SharePoint Online; AVD Azure virtual desktop optional.`,
    ...
    {
      title: 'Резюме главы',
      content: `1. **Golden image + Autopilot** — не manual install
...
8. **On/Offboarding** — checklists с HR timing

> Следующая глава: **Playbooks** — пошаговые runbooks для типовых инцидентов.`,
    },
  ],
  practice: [

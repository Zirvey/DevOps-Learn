...
const translation: ChapterTranslation = {
  title: 'OPNsense — operations, HA, and troubleshooting',
  duration: '6–8 hours',
  description:
    'Day-2 ops, CARP HA, backup/restore, firmware lifecycle, monitoring (SNMP/Netdata/Grafana), REST API, Ansible/Terraform, performance tuning, DR runbooks, outage playbooks, and production case studies',
  sections: [
    {
      title: 'Day-2 operational model for OPNsense',
      content: `**Day-2** is everything that happens after initial firewall setup: availability, changes, monitoring, recovery, and audit. OPNsense in SMB/MSP often runs for years on one appliance — without formalized processes, any failover or upgrade becomes an outage.

**Five pillars of OPNsense operations:**

| Pillar | Goal | Typical artifacts |
|--------|------|-------------------|
| **Availability** | RTO/RPO for edge | CARP HA, multi-WAN, runbooks |
| **Change** | Predictable changes | CM tickets, config diff, maintenance window |
| **Observability** | See degradation before outage | SNMP, syslog, Netdata, Grafana |
| **Recovery** | Restore without "magic" | config.xml backups, DR VM, documented restore |
...
| **MSP NOC** | Tier-1 alerts, runbook escalation |

**OPNsense vs FortiGate in ops:** no FortiCare SLA — you plan HA, backups, and security advisory response yourself. Plus: full access to FreeBSD shell, config.xml, and API without vendor gatekeeping.

**As-built documentation (must-have):**
- Physical + logical diagram (WAN, LAN, VLANs, sync link)
- IP plan with gateway VIP (CARP) and real node IPs
...
- Admin users in password manager (not plain in config.xml)

**Edge health metrics (baseline):**
- WAN latency/jitter to 2+ probes
...
- Last successful backup timestamp`,
    },
    {
      ...
      content: `Use as a **daily/weekly/monthly** checklist. Adapt frequency to client SLA.

**Daily (automated + 5 min human review):**
- [ ] config.xml backup successful (check timestamp in monitoring/SFTP)
- [ ] CARP: expected MASTER on node1 (or documented primary)
- [ ] WAN link up, gateway ping OK
- [ ] Disk usage < 80% on all nodes
- [ ] No critical alerts in Zabbix/Grafana (states spike, interface down)
- [ ] Suricata/IDS not in permanent blocking mode without ticket
...
- [ ] Check scheduled firmware status (pending security release?)
- [ ] Verify syslog ingestion on central SIEM (event count not zero)
- [ ] Test read-only API token still valid

**Monthly:**
- [ ] OPNsense security advisories + FreeBSD CVE for critical packages
...
- [ ] Plugin versions aligned between HA nodes

**Quarterly:**
- [ ] **CARP failover test** in maintenance window
- [ ] **Restore test**: config.xml on lab VM
- [ ] DR runbook walkthrough (tabletop)
- [ ] Packet capture baseline (sanity check paths)
- [ ] Review hw.igb/RSS tuning if throughput complaints

**On every change:**
- [ ] CM ticket with rollback plan
- [ ] Pre-change backup (manual + verify download)
- [ ] Change window + stakeholder notify (VPN blip during HA)
- [ ] Post-change verification checklist
- [ ] Config diff archived in git/S3
...
- [ ] Contract/ISP renewal, public IP inventory`,
      code: {
        language: 'shell',
        caption: 'Quick daily health check via SSH (one node)',
        code: `# version and uptime
opnsense-version
uptime

# CARP VIP status (if HA)
...
# last backup in /conf if local copy
ls -lt /root/config-backup-* 2>/dev/null | head -3`,
      },
    },
    {
      title: 'CARP HA: Virtual IPs and architecture',
      content: `**CARP (Common Address Redundancy Protocol)** — FreeBSD implementation of redundant gateway. Clients use **Virtual IP (VIP)** as default gateway; when MASTER fails, BACKUP raises VIP on its interface.

**Typical active/passive topology:**
...
\\\`\\\`\\\`
    ...
    │ MASTER │  │ BACKUP │
    │ WAN    │  │ WAN    │
    │ 1.2.3.4│  │ 1.2.3.5│  ← real IPs (different)
    └────┬───┘  └───┬────┘
...
**VIP components:**
- **vhid** — Virtual Host ID (1–255), same on pair for given VIP
- **advbase / advskew** — MASTER priority (lower skew = preferred master)
- **password** — CARP authentication (not empty in production!)
- **interface** — LAN, WAN, or DMZ where shared gateway needed

**VIP setup (GUI):**
1. Interfaces → Virtual IPs → Add
2. Type: **CARP**
3. Interface: LAN (or WAN)
4. Address: VIP gateway (e.g. 192.168.1.1/24)
5. Virtual Host ID: unique per VIP subnet
6. Advertising Frequency: base 1, skew 0 on intended master, skew 100 on backup

**WAN CARP nuances:**
- ISP must provide **multiple public IPs** or accept ARP from VIP MAC
- Some ISPs/CPE block — then HA only on LAN, WAN via external router
- PPPoE HA is harder — often multi-WAN with different ISPs per node

**Real IP per node** needed for:
- SSH management (not via VIP during split-brain)
- Sync link (pfsync, XMLRPC)
- Outbound SNAT if WAN VIP not used

**Document in as-built:**
...
| fw2 | 1.2.3.5 | 192.168.1.3 | 192.168.1.1 | 100 | BACKUP |`,
    ...
    {
      title: 'CARP: sync interface, pfsync, and config sync',
      content: `**Stateful HA** requires **state table** synchronization — without it TCP sessions break on failover.

**pfsync** — protocol replicating pf states between nodes.

**Setup (System → High Availability):**
- Enable **Synchronize states**
- Synchronize interface: **dedicated OPT/sync** (recommended) or LAN
- Peer IP: real IP of other node on sync subnet (e.g. 10.255.255.1 ↔ 10.255.255.2)

**Sync interface best practices:**
- Dedicated **1 Gbps direct cable** between nodes (not through production switch if possible)
- Separate subnet: 10.255.255.0/30
- Firewall rules: allow **pfsync** (protocol PFSYNC) and **CARP** between node IPs
- Do not route sync subnet into production VLANs

**Config sync (XMLRPC):**
- System → High Availability → **Synchronize configuration**
- Remote node IP: management/sync IP of peer
- Sync items: aliases, rules, NAT, VPN, users (everything except node-specific: hostname, interface IPs)
- **Node-specific exclusions:** real WAN/LAN IPs, CARP skew, hostname

**HA pair initial setup order:**
1. Configure fw2 as standalone clone (restore config from fw1, change real IPs)
2. Connect sync link
3. On fw1: enable pfsync + config sync peer = fw2 sync IP
4. On fw2: mirror settings peer = fw1
5. Push config fw1 → fw2 (or wait auto sync)
6. Verify rules identical: \\\`pfctl -vvsr | wc -l\\\` on both
7. Failover test

**Firewall rules for HA (example logic):**
...
**Bandwidth:** pfsync traffic = function of connection rate × state size. High CPS environments (many short TCP) — monitor sync link utilization.`,
      code: {
        language: 'shell',
        caption: 'Check pfsync and CARP on shell',
        code: `# CARP status per interface
ifconfig carp1
ifconfig carp2
...
# rules with CARP/pfsync (should be early in rule set)
pfctl -vvsr | grep -E 'carp|pfsync'`,
      },
    },
    {
      title: 'CARP: preempt, advskew, and failover testing',
      content: `**Preempt** — when MASTER returns after recovery, it becomes MASTER again (reclaims VIP). Default CARP behavior: lower advskew wins.

**advskew tuning:**
- Intended primary: skew **0–10**
- Backup: skew **100–254**
- Equal skew — tie-break by IP

**Forced role change (maintenance):**
- Temporarily raise skew on primary → backup becomes MASTER
- Or shutdown primary in window
- **Do not** disable sync link without isolate procedure
...
| 2 | From workstation: continuous ping to gateway VIP | 0% loss ideal |
| 3 | Initiate long TCP (SSH, RDP) through firewall | Session active |
| 4 | \\\`ifconfig carp1 down\\\` on MASTER or power off | VIP moves < 3s |
...
**Maintenance failover without power off:**
\\\`\\\`\\\`
# on current MASTER — demote self (temporary skew)
# via GUI Virtual IP or ifconfig tuning
# preferred: GUI maintenance mode plugins if installed
\\\`\\\`\\\`

**VPN during failover:** WireGuard/OpenVPN on VIP — clients reconnect; site-to-site IPsec may need dpd/rekey. Document expected VPN blip 5–30 sec.
...
- Zabbix: two host items per node, trigger on role mismatch duration`,
      code: {
        language: 'shell',
        caption: 'Simulate failover and verify preempt',
        code: `# on MASTER — check skew
ifconfig carp1 | grep advskew

# demote: increase skew (example — careful in production)
# ifconfig carp1 vhid 1 advskew 200

# verify peer took MASTER
ssh root@fw2 'ifconfig carp1 | grep carp'

# restore skew on primary
# ifconfig carp1 vhid 1 advskew 0

# continuous ping from client (start BEFORE failover)
# ping -D 192.168.1.1`,
      },
    },
    {
      title: 'HA pitfalls: asymmetric routing',
      content: `**Asymmetric routing** — when request and response go through different firewalls. In an HA pair this breaks stateful firewall: one node sees only half-connection, drops or weird NAT.

**Typical scenarios after failover:**

| Scenario | Symptom | Cause |
|----------|---------|-------|
| Stale ARP on switch | Partial connectivity | Switch MAC table points to old MASTER MAC |
...
- [ ] Default gateway for all VLANs = **LAN VIP**, not real node IP
- [ ] SNAT outbound uses **WAN VIP** or interface IP mode consistent on both nodes
- [ ] Static routes on upstream router point to VIP
...
- For cloud: consider **cold standby** instead of CARP if hypervisor blocks CARP MAC`,
    ...
    {
      title: 'HA pitfalls: state sync and split-brain',
      content: `**pfsync failure** — failover happens but states not synchronized → mass TCP reset, users "kicked" from sessions.

**Causes of pfsync break:**
- Sync link down / wrong peer IP
- Firewall rule blocks pfsync between nodes
- One node overloaded — pfsync drops
- Version mismatch after partial upgrade
- MTU issue on sync link (fragmentation)

**Split-brain** — both nodes consider themselves MASTER for one VIP.
...
**Cloud HA caveats:**
- AWS/Azure/GCP: CARP often **does not work** out of the box (MAC restrictions)
...
**Testing state sync:**
\\\`\\\`\\\`
# on MASTER — create SSH session
# on BACKUP before failover:
pfctl -ss | grep <your_src_ip>  # state should appear
\\\`\\\`\\\`
If empty on backup — pfsync broken **before** failover test matters.`,
    },
    {
      title: 'Backup and restore: config.xml',
      content: `**All OPNsense configuration** lives in \\\`/conf/config.xml\\\` (XML). Rules, NAT, VPN, users, plugins — one file.

**GUI backup:**
- System → Configuration → Backups → **Download**
- Optional encryption password (recommended for offsite)
...
3. Verify interface assignments (especially after restore on different hardware)
...
**Revision history:** System → Configuration → History — UI-level rollback (not substitute for offsite backup).`,
      code: {
        language: 'shell',
        caption: 'Backup, validate, and diff config.xml',
        code: `# backup with timestamp
cp /conf/config.xml /root/config-backup-$(date +%Y%m%d).xml

# validate XML
xmllint --noout /conf/config.xml && echo OK

# checksum for monitoring
sha256 /conf/config.xml

# diff two versions (on admin workstation)
diff -u config-before-change.xml config-after-change.xml | less`,
      },
    },
    {
      title: 'Backup: automation (SFTP, S3, cron)',
      content: `**Scheduled backup** — System → Configuration → Backups:
- Google Drive plugin
- **Nextcloud**
- **SFTP/SCP** to central backup server
- Email (small configs only — not recommended for secrets)
...
**Cron on OPNsense (if custom script):**
\\\`\\\`\\\`bash
...
- \\\`configctl config diff\\\` or xmlstarlet to sanitize passwords before commit

**Backup monitoring:**
- External check: file age < 25h on backup server
- SHA256 change alert (unexpected drift without CM ticket)
- Separate backup per HA node? **One logical config** enough if sync works; backup **both** if node-specific drift suspected
...
- Test restore quarterly per customer tier`,
      code: {
        language: 'shell',
        caption: 'Automated backup + gpg + SCP',
        code: `#!/bin/sh
set -e
...
rm -f "$DEST" "\${DEST}.gpg"`,
    ...
    {
      title: 'Firmware upgrade: procedure',
      content: `**In-place upgrade** — standard for OPNsense minor releases. Major jumps — read release notes twice.

**Pre-upgrade checklist:**
- [ ] Backup config.xml (verified download)
- [ ] Read [release notes](https://opnsense.org/releases/) and forum sticky
...
**Kernel vs base:** sometimes two reboot cycles — do not panic, wait full boot.`,
      code: {
        language: 'shell',
        caption: 'CLI firmware update and post-check',
        code: `# check pending updates
opnsense-update -p
...
service configd status`,
    ...
    {
      title: 'Firmware: rollback and plugin lifecycle',
      content: `**Rollback** in OPNsense is not "one click" like VM snapshot — plan rollback path **before** upgrade.

**Rollback strategies:**

| Method | RTO | When to use |
|--------|-----|-------------|
...
**Downgrade firmware:** officially not supported on same partition — reinstall previous version ISO + restore config from **pre-upgrade** backup.
...
- Rollback executed? Y/N`,
    ...
    {
      title: 'Monitoring: SNMP, Netdata, and exporters',
      content: `**Observability stack** for OPNsense is typically hybrid: built-in reporting + external NMS.

**SNMP (built-in):**
- System → Settings → **SNMP**
- Version: **SNMPv3** (authPriv) — not v2c public in production
- Modules: pf, interfaces, CARP (via custom), disk, load
- Bind to management interface only

**Useful OID categories:**
| Category | Use |
|----------|-----|
...
- Real-time metrics on node
- Dashboard localhost:19999 — **do not expose on WAN**
- Parent-child streaming to central Netdata for MSP

**Prometheus node_exporter pattern:**
- Telegraf or custom **exec** script → Pushgateway/Prometheus
...
**Alert thresholds (SMB starting points):**
...
**Do not monitor only WAN IP ping** — monitor gateway functionality: DNS resolve + HTTP probe through firewall.`,
      code: {
        language: 'shell',
        caption: 'SNMP walk and pf states for monitoring script',
        code: `# SNMPv3 walk (from admin workstation)
snmpwalk -v3 -l authPriv -u monitor -a SHA -A 'SECRET' \\
  -x AES -X 'SECRET' 192.168.99.1 system

# pf states count for custom exporter
...
if ifconfig carp1 | grep -q carp:1; then echo MASTER; exit 0; else echo BACKUP; exit 1; fi`,
    ...
    {
      title: 'Monitoring: Telegraf and Grafana',
      content: `**Telegraf** on OPNsense (via FreeBSD pkg or sidecar) — flexible metrics pipeline → InfluxDB/Prometheus/Mimir.

**Typical MSP architecture:**
...
- Internal probe from monitoring VLAN: HTTP → internal app through full path`,
    ...
    {
      title: 'Remote syslog and SIEM integration',
      content: `**Centralized logging** is critical for security audit and outage RCA.
...
- Protocol: TCP (reliable) or TLS if supported
...
- Sampling on high-traffic allow rules
...
**Test monthly:** inject test log line or trigger deny rule → verify arrival in SIEM < 60s.`,
      code: {
        language: 'shell',
        caption: 'Check syslog and local log files',
...
# test remote syslog (on rsyslog server)
logger -n 10.0.10.100 -P 514 -t opnsense-test "syslog pipeline OK"`,
      },
    },
    {
      title: 'REST API: authentication and endpoints',
      content: `**OPNsense API** — REST over HTTPS, JSON payloads. Coverage grows each release; not 100% GUI parity — verify per task.
...
**Rate limits:** no hard vendor limit — do not hammer search endpoints in loop.`,
      code: {
        language: 'shell',
        caption: 'REST API: authentication and firmware status',
...
- SSL verify failures — use internal CA cert on automation host`,
      code: {
        language: 'shell',
        caption: 'REST API: search rules and apply firewall',
        code: `KEY="api-key"
SECRET="api-secret"
BASE="https://192.168.99.1"
...
curl -sk -u "$KEY:$SECRET" \\
  ...
  -d '{}' | jq .`,
    ...
    {
      title: 'Ansible and Terraform for OPNsense',
      content: `**Ansible** — most mature path for OPNsense automation via **ansibleguy/opnsense** or **patrickjahns/opnsense_plugin** community collections.
...
- Survey: customer_id, ticket_number mandatory`,
    ...
    {
      title: 'Performance tuning: NIC, RSS, and hw.igb',
      content: `**Throughput complaints** on OPNsense often = NIC driver tuning, not "buy faster CPU".

**Intel igb/ixgbe (common):**
- Loader tunables in \\\`/boot/loader.conf.local\\\`:
\\\`\\\`\\\`
...
4. Reboot if loader.conf changed`,
      code: {
        language: 'shell',
        caption: 'NIC tuning and interrupt load check',
        code: `# current loader tunables
...
watch -n1 'pfctl -si | grep current'`,
    ...
    {
      title: 'Performance tuning: state limits and memory',
      content: `**pf state table** — each TCP/UDP flow = state entry. Exhaustion → new connections fail, "random" app breakage.
...
**Monitoring:** alert at 70/90% sustained — investigate before hitting 100%.`,
    ...
    {
      title: 'Disk, ZFS health, and SMART',
      content: `**Disk failure** on edge firewall = outage if no HA. Monitor proactively.
...
2. Single: maintenance window, USB reinstall, restore config.xml`,
      code: {
        language: 'shell',
        caption: 'ZFS health and disk space audit',
...
smartctl -H /dev/ada0`,
    ...
    {
      title: 'Crash dumps and kernel panics',
      content: `**Kernel panic** on firewall — rare but memorable. Prepare capture before second occurrence.
...
**VM:** increase host monitoring — hypervisor pause/stop events.`,
    },
    {
      ...
      content: `**Patch cadence** for OPNsense — balance security vs change risk.
...
**Dev/test:** lab mirror production version — test upgrade path before prod.`,
    ...
    {
      title: 'Change management for OPNsense',
...
**Post-implementation review (PIR):** if incident — blameless, update runbook.`,
    },
    {
      title: 'DR runbooks',
      content: `**Disaster Recovery** — when site total loss, ransomware on hypervisor, or double hardware failure.
...
**Documentation storage:** runbook copies **offsite** — not only on OPNsense.`,
    },
    {
      ...
      content: `**Packet capture** — last accurate tool when logs are insufficient.
...
**Capture without "melting" production:**
...
**Workflow "app not working through firewall":**
...
**Tools:** Wireshark, tcpdump, **termshark** on jump host.`,
      code: {
        language: 'shell',
        caption: 'tcpdump: capture and rotation',
...
tcpdump -nr /tmp/wan.pcap | head -50`,
    ...
    {
      title: 'Outage playbooks: internet, VPN, and DHCP',
      content: `### Playbook 1: "No internet" (all users)
...
**Forecast template:** users × 15 states + servers × 50 + 20% margin.`,
    },
    {
      ...
      content: `**MSP** manages dozens of OPNsense instances — standardization = margin.
...
**Billing:** per-device monitoring + backup storage + change hours.`,
    ...
    {
      title: 'Labs: practical scenarios',
...
A: CARP — native FreeBSD for OPNsense HA pair. VRRP not used in stock OPNsense.

**Q: Can you do active-active HA?**
A: Classic CARP — active/passive. Active/active multi-WAN possible but not same VIP active on both for same subnet.

**Q: How often to backup?**
A: Daily automated minimum; before every change manual.

**Q: Does API cover 100% GUI?**
A: No — check endpoint for specific task; some plugin settings GUI only.

**Q: Suricata on WAN — performance hit?**
A: Significant on SMB hardware — plan 30–50% throughput reduction IPS mode.

**Q: OPNsense in AWS HA?**
A: CARP problematic — consider single instance + automation restore or BGP solutions.

**Q: pfsync over LAN OK?**
...
**Q: Hardware replacement — interface names changed?**
A: Re-assign interfaces in console menu before full restore works.`,
    },
    {
      title: 'Interview Q&A',
      content: `**Q1: Describe CARP pair failover from start to end.**
...
**Q2: asymmetric routing after failover — how to diagnose?**
...
**Q3: HA pair upgrade order?**
A: Backup node first, verify, then primary, plugins after core both, optional failover test between.

**Q4: config.xml restore on new hardware?**
...
**Q5: states table full — what to do?**
...
A: Restored config passes checklist: WAN, NAT, VPN, critical rules, monitoring, backup re-established, documented RTO met.`,
    ...
    {
      title: 'Case study 1: asymmetric routing after CARP failover',
      content: `**Context:** SMB manufacturing, 120 users, OPNsense HA CARP pair on Proxmox, dual VLAN (Users + Servers).
...
- Post-mortem added to quarterly failover checklist item "verify DB flows"`,
    ...
    {
      title: 'Case study 2: firmware upgrade on HA pair without procedure',
      content: `**Context:** MSP client retail chain, 40 stores, HQ OPNsense HA — remote branches single node.
...
**Lessons:** HA doubles complexity — **procedure is availability**; tooling enforce beats policy document.`,
    ...
    {
      title: 'Case study 3: MSP multi-site restore after ransomware',
      content: `**Context:** MSP hosting Proxmox cluster per region; 25 OPNsense VMs; centralized SFTP backups gpg encrypted; no HA on small clients.
...
**Lessons:** MSP multi-tenant — **backup without tested restore is vanity**; ransomware validates DR not HA alone.`,
  ...
  practice: [
    'Deploy CARP HA pair in Proxmox (2 VMs + sync link): LAN VIP, failover test with ping and SSH session',
    'Configure daily config.xml backup to SFTP + gpg; write external check "file age < 25h"',
    'Perform firmware upgrade on BACKUP node first in HA pair per documented procedure',
    'Set up SNMPv3 + export pf states to Zabbix or Grafana (exec script)',
    'Configure syslog forward to Graylog; create alert on CARP MASTER change',
    'Create API user, automate nightly backup pull via curl on Linux jump host',
    'Tuning lab: baseline iperf3 through firewall, then adjust hw.igb tunables and compare',
    'DR exercise: restore config.xml on new VM, measure RTO, document gaps',
    'Outage simulation: break outbound NAT, walk through "no internet" playbook with tcpdump',
    'Tabletop: asymmetric routing case — set wrong gateway on server, reproduce, fix',
  ],
  resources: [
  ...
  quiz: [
    ...
    {
      question: 'pfsync synchronizes:',
      options: [
        'State table between HA nodes',
        'config.xml only',
        'DNS records only',
        'DHCP leases only',
      ],
      answer: 'State table between HA nodes',
    },
    {
    ...
    {
      question: 'HA pair firmware upgrade order:',
      options: [
        'BACKUP node first, then MASTER',
        'Both simultaneously',
        'MASTER first always',
        'Plugins only',
      ],
      answer: 'BACKUP node first, then MASTER',
    },
    {
      question: 'CARP split-brain means:',
      options: [
        'Both nodes are MASTER for one VIP',
        'No internet connectivity',
        'DNS failure',
        'DHCP pool exhausted',
      ],
      answer: 'Both nodes are MASTER for one VIP',
    },
    {
      question: 'Asymmetric routing after failover causes:',
      options: [
        'Stateful firewall drops / broken TCP sessions',
        'VPN speedup',
        'Automatic DNS fix',
        'PoE shutdown',
      ],
      answer: 'Stateful firewall drops / broken TCP sessions',
    },
    {
      question: 'SNMP in production OPNsense should use:',
      options: [
        'SNMPv3 authPriv, not v2c public',
        'v2c public on WAN',
        'UDP 161 open to internet',
        'Disable SNMP always',
      ],
      answer: 'SNMPv3 authPriv, not v2c public',
    },
    {
      question: 'The 3-2-1 backup rule for MSPs means:',
      options: [
        '3 copies, 2 media types, 1 offsite',
        '3 servers, 2 disks, 1 tape',
        '3 VPN tunnels, 2 WAN links',
        '3 VLANs, 2 firewalls',
      ],
      answer: '3 copies, 2 media types, 1 offsite',
    },
    {
    ...
    {
      question: 'advskew in CARP determines:',
      options: [
        'MASTER priority (lower skew wins)',
        'DNS TTL',
        'DHCP lease time',
        'VPN listen port',
      ],
      answer: 'MASTER priority (lower skew wins)',
      explanation: 'Primary: skew 0–10; backup: 100–254.',
    },
  ],
}

...
const translation: ChapterTranslation = {
  title: 'FortiGate - VPN (IPsec and SSL)',
  duration: '10–12 hours',
  description:
    'Site-to-Site IPsec with AWS, SSL-VPN with AD/LDAP, FortiClient EMS, split tunnel, hardening - production SMB remote',
  sections: [
    {
      title: 'VPN types on FortiGate',
      content: `FortiGate supports several VPN technologies. For SMB offices in 2026, two main ones are relevant:

| Type | Protocol | Script | Client |
|-----|----------|----------|--------|
| **IPsec Site-to-Site** | IKEv1/v2 + ESP | Office ↔ AWS/Azure, office ↔ branch | Automatic (gateway-to-gateway) |
| **SSL-VPN (Tunnel Mode)** | TLS 1.2/1.3 | Remote employees | FortiClient |
| **IPsec Dial-up** | IKEv2 | Remote users (SSL alternative) | FortiClient/native OS |
| **SSL-VPN Web Mode** | HTTPS | Browser-only access (limited) | Web browser |

**Do not use in 2026:**
- PPTP — broken cryptography
- L2TP without IPsec - insecure
- SSL-VPN Web Mode as the main remote access (limited functionality)

**SMB Selection:**
- **Remote workers** → SSL-VPN Tunnel Mode + FortiClient + AD/LDAP + MFA
- **Cloud connectivity** → IPsec Site-to-Site to AWS VGW/TGW
- **Branch office** → IPsec Site-to-Site between FortiGate

**Licensing:** SSL-VPN users limited by hardware (FG-60F: 200 max tunnel mode). Check the datasheet.`,
    },
    {
      title: 'IPsec fundamentals: Phase 1 and Phase 2',
      content: `**IPsec VPN** consists of two phases of negotiation (IKE):

**Phase 1 (IKE SA)** - secure channel between gateways:
- Authentication: Pre-Shared Key (PSK) or Certificates
- Encryption: AES-256-GCM (recommended)
- Integrity: SHA-256 (if not GCM)
- DH Group: 14 or 19 (ECP-256)
- Lifetime: 86400s (24h)
- Mode: Main (IKEv1) or IKEv2 (preferred)

**Phase 2 (IPsec SA)** — tunnel parameters for data:
...
- **Tunnel interface** = virtual interface for routing

**Route-based vs Policy-based:**
- **Route-based** (recommended): tunnel interface + static route. Easier troubleshooting, SD-WAN compatible
- **Policy-based**: phase2 selectors determine traffic. Legacy, avoid for new deployments`,
    },
    {
      title: 'IPsec Site-to-Site: office ↔ branch',
      content: `**Scenario:** head office (FG-HQ, 192.168.0.0/16) ↔ branch (FG-Branch, 192.168.100.0/24).

**On FG-HQ (GUI VPN → IPsec Wizard → Site to Site):**

1. **Name:** Branch-Office-Tunnel
2. **Remote Gateway:** static IP of the branch office (or DDNS)
3. **Outgoing interface:** wan1
4. **Authentication:** PSK (minimum 32 random chars) or Certificate
...
**On FG-Branch:** mirror configuration (local/remote swapped).

**Firewall policies (both sides):**
...
**Check:** \\\`diagnose vpn tunnel list\\\`, ping remote subnet.`,
      code: {
        language: 'text',
...
config vpn ipsec phase2-interface
    ...
    {
      title: 'IPsec Site-to-Site with AWS',
      content: `**Scenario:** SMB office (FortiGate) ↔ AWS VPC (10.50.0.0/16) via Site-to-Site VPN.

**AWS side:**
1. **Virtual Private Gateway (VGW)** or **Transit Gateway (TGW)** attached to VPC
2. **Customer Gateway (CGW):** register FortiGate WAN IP + BGP ASN (if BGP) or static
...
**FortiGate side (2 tunnels recommended):**
...
**Firewall policy on FG:** VLAN-Users → AWS-Tunnel, dest 10.50.0.0/16.

**Monitoring:** CloudWatch VPN metrics + FG \\\`diagnose vpn ike gateway list\\\`.`,
      code: {
        language: 'text',
        caption: 'CLI: static route to AWS VPC over IPsec',
    ...
    },
    {
      ...
      content: `**Route-based VPN (recommended):**
- Phase2 creates a tunnel interface (e.g. \\\`Branch-Office-P1\\\`)
- Traffic direction is determined by **routing table** + **firewall policies**
- \\\`net-device enable\\\` on phase1 (default in wizard)
- SD-WAN compatible
- It’s easier to add/remove subnets (just a new route)

**Policy-based VPN (legacy):**
- Phase2 selectors (src/dst subnet) determine traffic
- No tunnel interface
- Each subnet pair = separate phase2
- Does not work with SD-WAN
- More difficult troubleshooting

**Migration policy → route-based:**
1. Create new route-based tunnel
2. Parallel run (test)
3. Migrate routes
4. Delete old policy-based

**For new deployments:** always route-based. AWS VPN Wizard generates route-based config.`,
    },
    {
      title: 'SSL-VPN: architecture and modes',
      content: `**SSL-VPN** uses TLS (port 443) for VPN tunnel. Passes through most firewalls and NAT.

**FortiGate SSL-VPN modes:**

| Mode | Description | Use case |
|------|----------|----------|
| **Tunnel Mode** | Full network access via virtual adapter | Remote workers (main) |
| **Web Mode** | Browser proxy, portal apps | Vendor temporary access |
| **Web + Tunnel** | Both | Mixed |
...
**Settings (GUI):** VPN → SSL-VPN Settings:
- Listen on: wan1 (or wan1 + wan2)
- Port: 443
- Server certificate: Fortinet factory or Let's Encrypt / corporate cert
...
**Capacity:** FG-60F supports 200 concurrent SSL-VPN tunnel mode users (check license).`,
    ...
    {
      title: 'SSL-VPN portal and tunnel mode policies',
      content: `**SSL-VPN Portal** determines what the user sees after logging in: bookmarks, tunnel settings, split tunneling.
...
- **Routing Address:** ADDR-Internal-All (which subnets push to client)
...
- Profiles: AV, IPS (recommended)`,
    },
    {
      ...
      content: `**For production SMB** - never local users for VPN. Use **LDAP/AD integration**.

**Setting up LDAP connector (GUI):**
1. User & Authentication → LDAP Servers → Create New
2. Name: AD-Corp
3. Server IP: 192.168.10.10 (DC)
4. Port: 389 (or 636 LDAPS)
...
**When offboarding:** disable AD account → VPN access revoked immediately (no FG change needed).`,
      code: {
        language: 'text',
        caption: 'CLI: LDAP server and user group',
...
config user group
    ...
    {
      title: 'FortiClient: deployment and configuration',
      content: `**FortiClient** is a free VPN client for FortiGate SSL-VPN and IPsec.
...
**Download:** fortinet.com/support/product-downloads or from the FG portal.`,
    },
    {
      title: 'FortiClient EMS and endpoint compliance',
      content: `**FortiClient EMS (Endpoint Management Server)** - centralized management of FortiClient: deploy, config, compliance check, quarantine.

**Compliance use case:**
- VPN access **only** if: AV updated, OS patched, disk encrypted, FortiClient running
...
**SMB sizing:** EMS free tier up to 25 endpoints. For 50 users - EMS-B100 license.
...
- Routing Address: ADDR-Internal-All (not 0.0.0.0/0)
- Internet Services: disabled

**FortiClient side:** «Allow local LAN access» — **disable** (prevents bypass).

**DNS split:** internal DNS (192.168.10.10) via VPN, public DNS direct. Configuration in SSL-VPN settings.`,
      code: {
        language: 'text',
    ...
    {
      title: 'Full tunnel for privileged users',
      content: `**Full tunnel portal** for IT admins, security team, users with access to sensitive data.
...
**Monitoring:** FortiAnalyzer report «VPN bandwidth by user» — detect abuse.`,
    ...
    {
      title: 'MFA: FortiToken and external providers',
      content: `**MFA is required** for VPN in production. Password only = unacceptable risk.
...
- Conditional Access: compliant device required`,
    },
    {
      ...
      content: `**Minimum set of policies for SSL-VPN:**
...
**Geo + VPN:** if all users are in one country - geo-block SSL-VPN port 443 from other countries (additional layer).`,
      code: {
        language: 'text',
    ...
    {
      title: 'Hardening VPN: production checklist',
      content: `**Required hardening checklist for SMB VPN:**

1. **MFA** on all VPN users (FortiToken or RADIUS MFA)
2. **AD groups** — VPN-Users, VPN-Admins (separate portals)
3. **No local users** for VPN (LDAP only)
...
7. **Strong PSK** for IPsec (32+ chars, rotate annually) or certificates
8. **IKEv2 only** (disable IKEv1)
9. **Crypto:** AES-256-GCM, SHA-256+, DH14+
10. **Geo-blocking** on WAN for VPN port (optional)
...
18. **Review** VPN users quarterly (AD group audit)`,
    },
    {
      ...
      content: `**IPsec does not rise - diagnostics:**
...
diagnose vpn ssl list`,
    ...
    },
    {
      ...
      content: `**Runbook: new remote employee - VPN access**
...
**Offboarding:** remove from VPN-Users group, revoke FortiToken, verify no active session.`,
    },
    {
      ...
      content: `**Runbook: VPN connected but file server/app unavailable**

**L1 checklist (5 min):**

| # | Check | Command/Action |
|---|-------|----------------|
| 1 | Tunnel IP received? | FortiClient status |
...
**Fix DNS:** SSL-VPN settings → DNS Server = 192.168.10.10 (internal DC).`,
    ...
    {
      title: 'AWS Site-to-Site VPN: Full Configuration (AWS + FortiGate)',
...
**Test:** from office ping EC2 private IP in VPC`,
      code: {
        language: 'text',
        caption: 'CLI: AWS Tunnel 1 (FortiGate) - Complete Example',
        code: `config vpn ipsec phase1-interface
    edit "AWS-Tunnel1-P1"
        set interface "wan1"
        set ike-version 2
...
config firewall policy
    ...
    {
      title: 'Azure VPN Gateway: Site-to-Site with FortiGate',
...
**vs PSK SMB:** PSK acceptable for single site-to-site if 32+ char random + annual rotation. Certs for multi-site enterprise.`,
    ...
    {
      title: 'IPsec Dial-up: remote users without SSL-VPN',
...
- EAP-RADIUS for AD auth`,
    ...
    {
      title: 'Redundant VPN: dual tunnel and SD-WAN integration',
...
**Benchmark:** iperf3 through VPN tunnel during pilot.`,
    ...
    {
      title: 'FortiOS 7.4 lab: VPN on VM',
...
**Optional:** AWS free tier VPC + VPN Connection to single FG VM.`,
    ...
    {
      title: 'Diagnose/debug directory (VPN)',
...
\\\`\\\`\\\``,
    },
    {
      ...
      content: `**Client:** consulting firm, 200 SSL-VPN users, FG-100F.
...
**Lesson:** size VPN by concurrent sessions, not employee count.`,
    },
    {
      ...
      content: `**Client:** e-commerce, office to AWS VPC for ERP.
...
**Post-fix:** failover in 8 seconds on Tunnel 1 disable test.`,
    },
    {
      ...
      content: `**Client:** manufacturer, B2B IPsec to 5 suppliers.
...
**Operational overhead:** CA management — use FortiAuthenticator or existing AD CS.`,
    ...
    {
      title: 'FAQ: 12 frequently asked questions (VPN)',
...
**Lab exam prep:** 2 VM FG site-to-site + SSL-VPN in 90 min.`,
    ...
    {
      title: 'Interview Q&A: 10 questions (VPN)',
...
\\\`\\\`\\\``,
    ...
    {
      title: 'Comparison of VPN technologies for SMB',
...
- Cloud-native startup without FG → WireGuard on VPS (simple, fast)
...
**Future:** ZTNA (FortiGate ZTNA tags) replacing traditional VPN for SaaS access. Monitor Fortinet ZTNA for hybrid model.`,
  ...
  practice: [
    'Setting up SSL-VPN tunnel mode with LDAP authentication on AD in lab',
    'Create 2 portals: split-tunnel (users) and full-tunnel (admins)',
    'Deploy FortiToken MFA for 3 test users',
    'IPsec site-to-site: FG-HQ ↔ FG-Branch (2 VM) with IKEv2 AES-256-GCM',
    'IPsec to AWS: raise VPN Connection in AWS free tier, connect FG VM - both tunnels',
    'Split tunnel setting: only 192.168.10.0/24 and 192.168.20.0/24',
    'Write a runbook onboarding remote worker (1 page)',
    'Troubleshoot: break PSK on IPsec, find the reason via ike debug',
    'Troubleshoot: remove user from AD group, verify VPN denied',
    'Compare bandwidth: full tunnel vs split tunnel (iperf3 test)',
    'Setting up FortiClient EMS trial (25 endpoints) with compliance check',
    'Azure VPN Gateway: document phase1 parameters for Azure RouteBased',
    'Certificate IPsec: setting up authmethod signature on the lab tunnel',
    'FortiOS 7.4 lab: full VPN lab VMware/KVM/ESXi (site-to-site + SSL)',
    'Redundant VPN: dual tunnel failover test with distance metrics',
  ],
  resources: [
  ...
  quiz: [
    {
      question: 'SSL-VPN on FortiGate often listens to the following port:',
      options: ['443', '22 only', '53 only'],
      answer: '443',
    },
    {
      question: 'IPsec Site-to-Site for:',
      options: ['Office ↔ cloud/office', 'Prints only', 'DHCP only'],
      answer: 'Office ↔ cloud/office',
    },
    {
      question: 'VPN auth to AD via:',
      options: ['LDAP/RADIUS', 'Only local users', 'Only PSK on the client'],
      answer: 'LDAP/RADIUS',
    },
    {
      question: 'MFA for VPN:',
      options: ['Mandatory in production', 'Never needed', 'Only for Guest Wi‑Fi'],
      answer: 'Mandatory in production',
    },
    {
      question: 'Split tunnel means:',
      options: ['Part of the traffic directly to the Internet', 'Everything is always via VPN', 'Disabling DNS'],
      answer: 'Part of the traffic directly to the Internet',
    },
    {
      question: 'FortiClient is used for:',

...
export const officeItQuizzes: Record<string, QuizQuestion[]> = {
  ...
  'opnsense-firewall-nat': [
    { question: 'OPNsense использует packet filter:', options: ['pf (FreeBSD)', 'iptables', 'nftables only', 'Windows Firewall'], answer: 'pf (FreeBSD)' },
    { question: 'Правила firewall в OPNsense обрабатываются:', options: ['Сверху вниз, первое совпадение', 'Снизу вверх', 'Случайно', 'Только на WAN'], answer: 'Сверху вниз, первое совпадение' },
    { question: 'Stateful inspection в pf означает:', options: ['Return traffic match по state table', 'Каждый пакет проходит все rules', 'NAT не нужен', 'Только UDP'], answer: 'Return traffic match по state table' },
    { question: 'Block vs Reject в pf:', options: ['Block — silent drop, Reject — ICMP error', 'Одно и то же', 'Reject только на WAN', 'Block только UDP'], answer: 'Block — silent drop, Reject — ICMP error' },
    { question: 'Port forward timeout при работающем NAT — типично:', options: ['Нет associated WAN pass rule', 'DHCP pool full', 'DNSSEC fail', 'CARP split-brain'], answer: 'Нет associated WAN pass rule', explanation: 'DNAT без разрешающего rule на WAN — классическая причина timeout.' },
  ...
  'opnsense-vpn': [
    { question: 'Для road warrior SMB в 2026 чаще выбирают:', options: ['WireGuard', 'PPTP', 'L2TP без IPsec', 'Telnet'], answer: 'WireGuard' },
    { question: 'WireGuard на OPNsense по умолчанию слушает UDP:', options: ['51820', '443', '22', '161'], answer: '51820' },
    { question: 'Site-to-site OPNsense ↔ AWS/Azure стандарт:', options: ['IPsec', 'PPTP', 'Telnet', 'SMB only'], answer: 'IPsec' },
    { question: 'Split tunnel VPN означает:', options: ['Часть трафика напрямую в internet', 'Всё через VPN', 'Отключение DNS', 'Только UDP'], answer: 'Часть трафика напрямую в internet' },
    { question: 'OpenVPN CSO (Client Specific Overrides) даёт:', options: ['Per-user routes и настройки', 'Только PSK', 'Только IPsec', 'Отключение MFA'], answer: 'Per-user routes и настройки' },
  ...
  'opnsense-services': [
    { question: 'Плагин os-suricata предоставляет:', options: ['IDS/IPS по сигнатурам', 'Только DHCP', 'Wi‑Fi controller', 'Почтовый relay'], answer: 'IDS/IPS по сигнатурам' },
    { question: 'Плагины OPNsense имеют префикс:', options: ['os-*', 'fg-*', 'vm-*', 'ad-*'], answer: 'os-*' },
    { question: 'HAProxy + os-acme-client используют для:', options: ['Reverse proxy с авто Let\'s Encrypt', 'Только DNS', 'Только VPN', 'Только DHCP'], answer: 'Reverse proxy с авто Let\'s Encrypt' },
    { question: 'Suricata IPS mode на SMB hardware:', options: ['Снижает throughput 30–50%', 'Ускоряет WAN вдвое', 'Не влияет на CPU', 'Отключает pf'], answer: 'Снижает throughput 30–50%' },
    { question: 'После major upgrade core OPNsense плагины:', options: ['Обновить в том же maintenance window', 'Никогда обновлять', 'Удалить все', 'Только reboot'], answer: 'Обновить в том же maintenance window' },
  ...
  'opnsense-operations': [
    { question: 'CARP в OPNsense используется для:', options: ['HA с shared Virtual IP', 'DNS cache', 'LDAP auth', 'PoE budget'], answer: 'HA с shared Virtual IP' },
    { question: 'pfsync синхронизирует:', options: ['State table между HA nodes', 'Только config.xml', 'Только DNS', 'Только DHCP leases'], answer: 'State table между HA nodes' },
    { question: 'Полная конфигурация OPNsense хранится в:', options: ['config.xml', 'registry.db', 'etcd', 'GPO AD'], answer: 'config.xml' },
    { question: 'Порядок firmware upgrade HA pair:', options: ['BACKUP node first, затем MASTER', 'Оба simultaneously', 'MASTER first всегда', 'Только plugins'], answer: 'BACKUP node first, затем MASTER' },
    { question: 'Split-brain CARP — когда:', options: ['Обе nodes MASTER для одного VIP', 'Нет интернета', 'DNS fail', 'DHCP pool full'], answer: 'Обе nodes MASTER для одного VIP' },
